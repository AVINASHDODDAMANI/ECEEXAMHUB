...
import { antennaVisualizationData } from "../../data/antenna-visualization-data";
import useVisualizationSteps from "../../hooks/useVisualizationSteps";
import VisualizationCard from "./VisualizationCard";
import StepExplanation from "./StepExplanation";
import RadiationPatternVisualizer from "./RadiationPatternVisualizer";
import AntennaSignalFlowVisualizer from "./AntennaSignalFlowVisualizer";
import AntennaPropagationVisualizer from "./AntennaPropagationVisualizer";

function renderVisual(visualType, activeStep) {
  if (visualType === "radiation-pattern") {
    return <RadiationPatternVisualizer type={visualType} activeStep={activeStep} />;
  }

  if (["propagation-modes", "ionosphere", "space-wave"].includes(visualType)) {
    return <AntennaPropagationVisualizer type={visualType} activeStep={activeStep} />;
  }

  return <AntennaSignalFlowVisualizer type={visualType} activeStep={activeStep} />;
}

function AntennaVisualizerComponent({ slug }) {
  const visual = antennaVisualizationData[slug];
      ...
      <div className="grid gap-4">
        ...
        <div className="grid gap-3 md:grid-cols-[1fr_0.72fr]">
          ...
          <aside className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
            <h4 className="text-sm font-black uppercase tracking-[0.12em] text-slate-500">Parameters</h4>
...
const AntennaVisualizer = memo(AntennaVisualizerComponent);
