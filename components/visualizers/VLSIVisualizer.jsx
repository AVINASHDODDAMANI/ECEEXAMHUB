...
import { vlsiVisualizationData } from "../../data/vlsi-visualization-data";
import useVisualizationSteps from "../../hooks/useVisualizationSteps";
import VisualizationCard from "./VisualizationCard";
import StepExplanation from "./StepExplanation";
import TransistorAnimation from "./TransistorAnimation";
import ChipLayoutVisualizer from "./ChipLayoutVisualizer";
import SignalFlowVisualizer from "./SignalFlowVisualizer";

function renderVisual(visualType, activeStep) {
  if (visualType === "transistor") {
    return <TransistorAnimation activeStep={activeStep} />;
  }

  if (visualType === "fabrication" || visualType === "layout") {
    return <ChipLayoutVisualizer type={visualType} activeStep={activeStep} />;
  }

  return <SignalFlowVisualizer type={visualType} activeStep={activeStep} />;
}

function VLSIVisualizerComponent({ slug }) {
  const visual = vlsiVisualizationData[slug];
      ...
      <div className="grid gap-4">
        ...
        <div className="grid gap-3 md:grid-cols-[1fr_0.72fr]">
          ...
          <aside className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
            <h4 className="text-sm font-black uppercase tracking-[0.12em] text-slate-500">Labels and Signals</h4>
...
const VLSIVisualizer = memo(VLSIVisualizerComponent);

export default VLSIVisualizer;
