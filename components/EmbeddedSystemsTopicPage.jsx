...
function EmbeddedChapterMenu({ topics = [], currentSlug }) {
  ...
  return (
    <div className="relative flex-none">
      <button
        ...
        aria-controls="embedded-topic-page-menu"
      >
        {isOpen ? (
      ...
      {isOpen ? (
        <div id="embedded-topic-page-menu" className="absolute right-0 z-30 mt-2 max-h-[72vh] w-[min(28rem,calc(100vw-2rem))] overflow-y-auto rounded-2xl border border-slate-200 bg-white p-2.5 shadow-[0_22px_60px_rgba(15,23,42,0.18)]">
          <div className="mb-2 rounded-xl border border-portal-200 bg-portal-50 px-3 py-2">
            <p className="text-xs font-black uppercase tracking-[0.12em] text-portal-700">Embedded Systems Chapters</p>
            <p className="mt-1 text-xs font-semibold leading-4 text-slate-700">Open any embedded topic page directly.</p>
          </div>
          <div className="grid gap-2">
            {topics.map((item, index) => (
              <Link
                key={item.slug}
                href={`/learn/embedded-systems/${item.slug}`}
...
export default function EmbeddedSystemsTopicPage({ topic }) {
  const relatedTopics = useMemo(() => getRelatedLearningTopics(topic.relatedTopics || []), [topic.relatedTopics]);
  const orderedTopics = useMemo(() => {
    const subject = getLearningSubject("embedded-systems");
  ...
  const faqItems = useMemo(
    () => [
      ...
      },
      {
        ...
        answer: "Revise the basic intuition first, use the visualization to remember the hardware-software flow, then practice one block-diagram and one concept question.",
      },
      {
    ...
    () => [
      ...
      },
      {
        ...
        itemListElement: [
          ...
          { "@type": "ListItem", position: 3, name: "Embedded Systems", item: "https://eceexamguide.vercel.app/subjects/embedded-systems" },
          { "@type": "ListItem", position: 4, name: topic.shortTitle, item: `https://eceexamguide.vercel.app/learn/embedded-systems/${topic.slug}` },
        ],
      },
    ],
    ...
    >
      <div className="mx-auto min-w-0 max-w-[1200px] pb-20">
        <nav aria-label="Breadcrumb" className="mb-4 flex flex-col gap-3 pt-1 sm:flex-row sm:items-start sm:justify-between">
          ...
          <EmbeddedChapterMenu topics={orderedTopics} currentSlug={topic.slug} />
        </nav>

        <header className="rounded-[28px] border border-slate-200 bg-white p-5 shadow-panel sm:p-6">
          <p className="text-[11px] font-bold uppercase tracking-[0.16em] text-portal-700">Embedded Systems</p>
        ...
        <article className="mt-5 grid gap-5">
          ...
          <TopicSection id="intuition" title="Basic Intuition">
            ...
            <div className="mt-4 grid gap-3 md:grid-cols-2">
              ...
              <div><h3 className="text-sm font-black uppercase tracking-[0.12em] text-slate-500">Important Blocks and Signals</h3><BulletList items={topic.keyConcepts} /></div>
            </div>
          </TopicSection>
          ...
          <TopicSection id="revision" title="Formula, Rule, and Revision Highlight">
          ...
          <TopicSection id="exam-focus" title="Worked Example and Exam Focus">
            ...
            <div className="mt-4 grid gap-3 md:grid-cols-2">
              <div><h3 className="text-sm font-black uppercase tracking-[0.12em] text-slate-500">Exam Pointers</h3><BulletList items={topic.examPointers} /></div>
              <div><h3 className="text-sm font-black uppercase tracking-[0.12em] text-slate-500">Common Mistakes</h3><BulletList items={topic.commonMistakes} bulletClassName="bg-rose-500" /></div>
            </div>
          </TopicSection>
        ...
        <div className="mt-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <Link href={previousTopic ? `/learn/embedded-systems/${previousTopic.slug}` : "/subjects/embedded-systems"} className="inline-flex justify-center rounded-xl border border-slate-200 bg-white px-5 py-3 text-sm font-bold text-slate-700 transition hover:bg-slate-50">
            {previousTopic ? `Previous ${previousTopic.title}` : "Back to Embedded Systems"}
          </Link>
          <Link href={nextTopic ? `/learn/embedded-systems/${nextTopic.slug}` : "/subjects/embedded-systems"} className="inline-flex justify-center rounded-xl bg-portal-600 px-5 py-3 text-sm font-bold text-white shadow-sm transition hover:bg-portal-700">
            {nextTopic ? `Next ${nextTopic.title}` : "Back to Embedded Systems"}
          </Link>
        </div>
      </div>
    </Layout>
