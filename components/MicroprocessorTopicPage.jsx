...
function MicroprocessorChapterMenu({ topics = [], currentSlug }) {
  ...
  return (
    <div className="relative flex-none">
      <button
        ...
        aria-controls="microprocessor-topic-page-menu"
      >
        {isOpen ? (
      ...
      {isOpen ? (
        <div id="microprocessor-topic-page-menu" className="absolute right-0 z-30 mt-2 max-h-[72vh] w-[min(28rem,calc(100vw-2rem))] overflow-y-auto rounded-2xl border border-slate-200 bg-white p-2.5 shadow-[0_22px_60px_rgba(15,23,42,0.18)]">
          ...
          <div className="grid gap-2">
            {topics.map((item, index) => (
              <Link
                key={item.slug}
                href={`/learn/microprocessors/${item.slug}`}
...
export default function MicroprocessorTopicPage({ topic }) {
  const relatedTopics = useMemo(() => getRelatedLearningTopics(topic.relatedTopics || []), [topic.relatedTopics]);
  const orderedTopics = useMemo(() => {
    const subject = getLearningSubject("microprocessors");
    return subject
  ...
  const faqItems = useMemo(
    () => [
      {
        question: `Why is ${topic.shortTitle} important for GATE ECE Microprocessors?`,
        answer: `${topic.shortTitle} connects hardware blocks, instruction flow, buses, and timing, which makes it useful for GATE ECE Microprocessors, PSU Microprocessors, 8085 notes, 8086 architecture, and university exam preparation.`,
      },
      {
        question: `How should I revise ${topic.shortTitle} for PSU Microprocessors?`,
        answer: "Revise the block flow first, use the visualization to remember sequence, then practice opcode, bus, timing, interrupt, or interfacing questions.",
      },
      {
      ...
      <div className="mx-auto min-w-0 max-w-[1200px] pb-20">
        <nav aria-label="Breadcrumb" className="mb-4 flex flex-col gap-3 pt-1 sm:flex-row sm:items-start sm:justify-between">
          ...
          <div className="flex items-center gap-2">
            <MicroprocessorChapterMenu topics={orderedTopics} currentSlug={topic.slug} />
        ...
        <header className="rounded-[28px] border border-slate-200 bg-white p-5 shadow-panel sm:p-6">
          <p className="text-[11px] font-bold uppercase tracking-[0.16em] text-portal-700">Microprocessors</p>
          ...
          <div className="mt-4 grid gap-3 text-sm text-slate-700 md:grid-cols-3">
            ...
            <div className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3"><p className="font-bold text-slate-950">Hardware use</p><p className="mt-1 leading-6">{topic.engineeringUse}</p></div>
          </div>
        </header>
        ...
        <article className="mt-5 grid gap-5">
          ...
          <TopicSection id="intuition" title="Basic Intuition">
            ...
            <div className="mt-4 grid gap-3 md:grid-cols-2">
              ...
              <div><h3 className="text-sm font-black uppercase tracking-[0.12em] text-slate-500">Important Registers and Buses</h3><BulletList items={topic.keyConcepts} /></div>
            </div>
          </TopicSection>

          <TopicSection id="visualization" title="Step-by-Step Visualization">
            <p className="mt-3 text-sm leading-7 text-slate-700 sm:text-base">
              This lightweight SVG animation explains {topic.shortTitle} for GATE ECE Microprocessors, PSU Microprocessors, 8085 notes, 8086 architecture, and university exam preparation.
            </p>
            <div className="mt-4"><MicroprocessorVisualizer slug={topic.slug} /></div>
          </TopicSection>
          ...
          <TopicSection id="formulas" title="Opcode, Formula, and Revision Highlights">
          ...
          <TopicSection id="related-topics" title="Related Microprocessor Topics">
            <p className="mt-3 text-sm leading-7 text-slate-700 sm:text-base">Continue in the same order as the Microprocessors hamburger menu.</p>
        ...
        <div className="mt-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <Link href={previousTopic ? `/learn/microprocessors/${previousTopic.slug}` : "/subjects/microprocessors"} className="inline-flex justify-center rounded-xl border border-slate-200 bg-white px-5 py-3 text-sm font-bold text-slate-700 transition hover:bg-slate-50">
            {previousTopic ? `Previous ${previousTopic.title}` : "Back to Microprocessors"}
          </Link>
          <Link href={nextTopic ? `/learn/microprocessors/${nextTopic.slug}` : "/subjects/microprocessors"} className="inline-flex justify-center rounded-xl bg-portal-600 px-5 py-3 text-sm font-bold text-white shadow-sm transition hover:bg-portal-700">
            {nextTopic ? `Next ${nextTopic.title}` : "Back to Microprocessors"}
          </Link>
        </div>
      </div>
    </Layout>
