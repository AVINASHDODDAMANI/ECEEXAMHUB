...
function DSPChapterMenu({ topics = [], currentSlug }) {
  ...
  return (
    <div className="relative flex-none">
      <button
        ...
        aria-controls="dsp-topic-page-menu"
      >
        {isOpen ? (
      ...
      {isOpen ? (
        <div id="dsp-topic-page-menu" className="absolute right-0 z-30 mt-2 max-h-[72vh] w-[min(28rem,calc(100vw-2rem))] overflow-y-auto rounded-2xl border border-slate-200 bg-white p-2.5 shadow-[0_22px_60px_rgba(15,23,42,0.18)]">
          <div className="mb-2 rounded-xl border border-portal-200 bg-portal-50 px-3 py-2">
            <p className="text-xs font-black uppercase tracking-[0.12em] text-portal-700">Digital Signal Processing Chapters</p>
            <p className="mt-1 text-xs font-semibold leading-4 text-slate-700">Open any DSP chapter page directly.</p>
          </div>
          <div className="grid gap-2">
            {topics.map((item, index) => (
              <Link
                key={item.slug}
                href={`/learn/dsp/${item.slug}`}
...
export default function DigitalSignalProcessingTopicPage({ topic }) {
  const relatedTopics = useMemo(() => getRelatedLearningTopics(topic.relatedTopics || []), [topic.relatedTopics]);
  const orderedTopics = useMemo(() => {
    const subject = getLearningSubject("dsp");
  ...
  const faqItems = useMemo(
    () => [
      {
        question: `Why is ${topic.shortTitle} important for GATE DSP?`,
        answer: `${topic.shortTitle} is useful for GATE DSP notes, Digital Signal Processing for PSU exams, university DSP notes, and DSP interview questions because it builds the link between signal intuition and numerical solving.`,
      },
      {
        question: `How should I revise ${topic.shortTitle} for PSU exams?`,
        answer: "Revise the intuition first, watch the visualization flow, then practice one numerical question and one conceptual question from the same chapter.",
      },
      {
    ...
    () => [
      {
        ...
        name: `${topic.shortTitle} Digital Signal Processing Notes`,
      ...
      <div className="mx-auto min-w-0 max-w-[1200px] pb-20">
        <nav aria-label="Breadcrumb" className="mb-4 flex flex-col gap-3 pt-1 sm:flex-row sm:items-start sm:justify-between">
          <ol className="flex flex-wrap items-center gap-2 text-sm text-slate-500">
            ...
            <li className="text-slate-300">/</li>
            <li><Link href="/subjects/digital-signal-processing" className="font-medium text-slate-600 transition hover:text-portal-700">Digital Signal Processing</Link></li>
            <li className="text-slate-300">/</li>
        ...
        <header className="rounded-[28px] border border-slate-200 bg-white p-5 shadow-panel sm:p-6">
          <p className="text-[11px] font-bold uppercase tracking-[0.16em] text-portal-700">Digital Signal Processing</p>
          ...
          <div className="mt-4 grid gap-3 text-sm text-slate-700 md:grid-cols-3">
            ...
            <div className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3"><p className="font-bold text-slate-950">Practical use</p><p className="mt-1 leading-6">{topic.engineeringUse}</p></div>
          </div>
        </header>
        ...
        <article className="mt-5 grid gap-5">
          ...
          <TopicSection id="intuition" title="Basic Intuition">
            ...
            <div className="mt-4 grid gap-3 md:grid-cols-2">
              ...
              <div><h3 className="text-sm font-black uppercase tracking-[0.12em] text-slate-500">Important Labels</h3><BulletList items={topic.keyConcepts} /></div>
            </div>
          </TopicSection>

          <TopicSection id="visualization" title="Step-by-Step Visualization">
            <p className="mt-3 text-sm leading-7 text-slate-700 sm:text-base">
              This lightweight SVG animation explains {topic.shortTitle} for GATE DSP notes, Digital Signal Processing for PSU exams, university DSP notes, and DSP interview questions.
            </p>
            <div className="mt-4"><DSPVisualizer slug={topic.slug} /></div>
          </TopicSection>
          ...
          <TopicSection id="formulas" title="Formula Highlight">
          ...
          <TopicSection id="related-topics" title="Related DSP Topics">
            <p className="mt-3 text-sm leading-7 text-slate-700 sm:text-base">Continue in the same order as the Digital Signal Processing hamburger menu.</p>
        ...
        <div className="mt-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <Link href={previousTopic ? `/learn/dsp/${previousTopic.slug}` : "/subjects/digital-signal-processing"} className="inline-flex justify-center rounded-xl border border-slate-200 bg-white px-5 py-3 text-sm font-bold text-slate-700 transition hover:bg-slate-50">
            {previousTopic ? `Previous ${previousTopic.title}` : "Back to Digital Signal Processing"}
          </Link>
          <Link href={nextTopic ? `/learn/dsp/${nextTopic.slug}` : "/subjects/digital-signal-processing"} className="inline-flex justify-center rounded-xl bg-portal-600 px-5 py-3 text-sm font-bold text-white shadow-sm transition hover:bg-portal-700">
            {nextTopic ? `Next ${nextTopic.title}` : "Back to Digital Signal Processing"}
          </Link>
        </div>
      </div>
    </Layout>
