...
const SUBJECT_SEO_ALIASES = {
  "Network Analysis": ["Network Theory", "Circuit Theory", "Network Analysis Notes"],
...
const PREVIOUS_PAPER_SITEMAP_CATALOG = [
  ...
  { exam: "BEL", startYear: 2023, endYear: 2014 },
...
export function generateTitle({
  ...
  if (type === "subject") {
    return `${formatNameWithAlias(subjectName || title)} Notes for ECE | GATE Preparation`;
  }

  if (type === "notes") {
    return `${formatNameWithAlias(subjectName || title)} Handwritten Notes for ECE | GATE Preparation`;
  }

  if (type === "topic") {
    return `${formatNameWithAlias(title)} Explained for GATE ECE | ${formatNameWithAlias(subjectName || "ECE Notes")}`;
  }

  if (type === "chapter") {
    return `${formatNameWithAlias(chapterTitle || title)} Notes for ECE | ${formatNameWithAlias(subjectName || "GATE Preparation")}`;
  }
...
export function generateDescription({
  ...
  if (type === "notes") {
    return `Access ${formatNameWithAlias(subjectName || title)} handwritten notes for ECE with chapter-wise theory, formulas, solved examples, quick revision points, and GATE exam preparation guidance.`;
  }

  if (type === "topic") {
    return `${formatNameWithAlias(title)} explained for ECE and GATE preparation with concepts, formulas, worked examples, revision tips, and related ${formatNameWithAlias(subjectName || "")} notes.`
      .replace(/\s+/g, " ")
      .trim();
  }
    ...
    return [
      {
        ...
        answer:
          "It becomes manageable once voltage, current direction, KCL, and KVL are clear. Most difficulty comes from choosing an equation method before reading the circuit carefully.",
      },
      {
...
export function getIndexableRoutes() {
  ...
  const paperRoutes = PREVIOUS_PAPER_SITEMAP_CATALOG.flatMap((item) =>
    buildYearRange(item.startYear, item.endYear).map((year) => ({
      ...
      priority: item.exam === "GATE" ? 0.7 : 0.6,
    }))
  );
  const officialPaperRoutes = officialPreviousPapers.map((paper) => ({
    path: slugifyPaperPath(paper.exam, paper.year),
    priority: 0.75,
  }));
  ...
  const routes = [
    ...
    { path: "/previous-year", priority: 0.8 },
    { path: "/notes", priority: 0.7 },
    { path: "/mcqs", priority: 0.7 },
    { path: "/practice", priority: 0.6 },
    { path: "/mock-tests", priority: 0.6 },
    { path: "/ece-exams", priority: 0.6 },
