...
import { grammarTopics } from "../data/grammar-hub";
import { documentTools } from "../data/document-tools";
import { getLearningSubject, getReadyLearningTopics } from "./learning-utils";
import { getPaperQuestions, slugifyPaper } from "./paper-document";
...
export const SITE_ALTERNATE_NAMES = ["ECEExamGuide"];
const LEGACY_SITE_NAMES = ["ECE Guide", "ECE Exam Guide Notes"];
...
const EDUCATIONAL_STATIC_ROUTES = [
  ...
  ...grammarTopics.map((topic) => `/grammar/${topic.slug}`),
  "/ac-circuit-analysis",
  "/ac-fundamentals",
...
export function normalizeMetaTitle(title = SITE_NAME) {
  ...
  const includesSiteName = [SITE_NAME, ...SITE_ALTERNATE_NAMES, ...LEGACY_SITE_NAMES].some(
...
const MIN_INDEXABLE_PAPER_QUESTIONS = 10;

function getPaperSolutionPath(paper = {}) {
  return `/solution/${paper.slug || slugifyPaper(paper.exam, paper.year)}`;
}

function hasIndexablePaperContent(paper = {}) {
  const paperQuestions = getPaperQuestions(seedQuestions, paper, "All Types");

  return Boolean(
    paper.pdfHref ||
      paper.isOfficialPdf ||
      Number(paper.questionCount || paperQuestions.length || 0) >= MIN_INDEXABLE_PAPER_QUESTIONS
  );
}

function hasOfficialPaperForYear(paper = {}) {
  return officialPreviousPapers.some(
    (officialPaper) =>
      officialPaper.exam === paper.exam &&
      Number(officialPaper.year) === Number(paper.year)
  );
}
...
export function getIndexableRoutes() {
  const redirectedRoutes = new Set([
    ...
    "/learn/networks/network-theorems-topic",
    "/learn/digital/boolean-algebra-and-kmaps",
  ]);
  ...
  const documentToolRoutes = [
    sitemapRoute("/document-tools", 0.85, "weekly"),
    ...documentTools.map((tool) =>
      sitemapRoute(`/document-tools/${tool.slug}`, tool.status === "enabled" ? 0.9 : 0.65, "weekly")
    ),
  ];
  ...
  const paperRoutes = PREVIOUS_PAPER_SITEMAP_CATALOG.flatMap((item) =>
    buildYearRange(item.startYear, item.endYear)
      .map((year) => ({
        exam: item.exam,
        year,
      ...
      .filter(hasIndexablePaperContent)
      .filter((paper) => !hasOfficialPaperForYear(paper))
  );
  const officialPaperRoutes = officialPreviousPapers.filter(hasIndexablePaperContent).map((paper) => ({
    path: getPaperSolutionPath(paper),
  ...
  const routes = [
    ...
    sitemapRoute("/previous-year-papers/bel", 0.95, "daily"),
