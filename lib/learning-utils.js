...
import { analogElectronicsSubject } from "../data/analog-electronics";
import { digitalElectronicsSubject } from "../data/digital-electronics";
import { signalsAndSystemsSubject } from "../data/signals-and-systems";
import {
  communicationLearningSubject,
  communicationTopicPageMap,
} from "../data/communication-topic-pages";
import { electromagneticTheorySubject } from "../data/electromagnetic-theory";
import { microprocessorsSubject } from "../data/microprocessors";
...
function toLearningSubject(subject, learningSlug) {
  return {
    slug: learningSlug,
    ...
    chapters: subject.chapters.map((chapter) => ({
      ...
      topics: chapter.topics.map((topic) => ({
        ...
        estimatedTime: topic.estimatedTime || "35 min",
...
const editableCoreLearningSubjects = {
  analog: toLearningSubject(analogElectronicsSubject, "analog"),
  digital: toLearningSubject(digitalElectronicsSubject, "digital"),
  signals: toLearningSubject(signalsAndSystemsSubject, "signals"),
  electromagnetics: toLearningSubject(electromagneticTheorySubject, "electromagnetics"),
  microprocessors: toLearningSubject(microprocessorsSubject, "microprocessors"),
};

function getMergedLearningSubjects() {
  const subjects = learningSubjects.map((subject) => {
    if (editableCoreLearningSubjects[subject.slug]) {
      return editableCoreLearningSubjects[subject.slug];
    }

    return subject.slug === communicationLearningSubject.slug
      ? communicationLearningSubject
      : subject;
  });
