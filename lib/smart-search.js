...
function hasPaperSearchIntent(normalizedQuery = "") {
  ...
  return hasPaperWord || (hasYear && hasKnownExam) || /bel.*(may|december).*2025|bel.*2025.*(may|december)/.test(compactQuery);
}

function getPaperIntentParts(normalizedQuery = "") {
  const compactQuery = normalizeCompact(normalizedQuery);
  const year = normalizedQuery.match(/\b(19|20)\d{2}\b/)?.[0] || compactQuery.match(/(19|20)\d{2}/)?.[0] || "";
  const month =
    [
      "january",
      "february",
