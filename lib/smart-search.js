...
function hasPaperSearchIntent(normalizedQuery = "") {
  ...
  return hasPaperWord || (hasYear && hasKnownExam);
}

function getPaperIntentParts(normalizedQuery = "") {
  const year = normalizedQuery.match(/\b(19|20)\d{2}\b/)?.[0] || "";
  const examAliases = [
    ["gate", "GATE"],
    ["isro", "ISRO"],
    ["bel", "BEL"],
  ...
  const exam = examAliases.find(([alias]) => normalizedQuery.split(" ").includes(alias))?.[1] || "";
...
function getPaperSearchAliases(exam = "", year = "") {
  ...
  const examAliases = {
    GATE: ["GATE ECE", "GATE Electronics", "Graduate Aptitude Test in Engineering"],
...
function getPaperSubtitle(paper) {
  if (paper.officialPaper?.role) {
    return `${paper.officialPaper.role} | ${paper.year}`;
  }
...
function buildSubjectItems() {
  ...
  const learningSubjectItems = getLearningSubjects().map((subject) => {
    ...
    return {
      ...
      description: directorySubject?.description || subject.description || "Open subject roadmap, chapters, topics, notes, and practice resources.",
      badge: "Subject",
...
function scoreItem(item, queryTokens, normalizedQuery) {
  ...
  if (paperIntent && item.type === "paper") {
    ...
    if (paperIntentParts.year && String(item.year) === paperIntentParts.year) {
      score += 120;
    }
...
export function getGroupedSmartSearchResults(query = "", index = [], maxPerGroup = 4) {
  ...
  const matchedItems = getSmartSearchResults(query, index, index.length).filter((item) => {
    ...
    const yearMatches = !paperIntentParts.year || itemText.includes(paperIntentParts.year);
