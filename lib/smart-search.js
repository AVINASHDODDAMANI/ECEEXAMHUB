...
const QUERY_INTENT_WORDS = new Set([
  "about",
  "an",
  "and",
  "article",
...
function getQueryCore(normalizedQuery = "") {
  const coreTokens = normalizedQuery
    .split(" ")
    .filter(Boolean)
    .filter((token) => !QUERY_INTENT_WORDS.has(token));

  return coreTokens.length ? coreTokens.join(" ") : normalizedQuery;
}

function getRankingQueryVariants(normalizedQuery = "") {
  return uniqueStrings([
    normalizedQuery,
    getQueryCore(normalizedQuery),
    canonicalSearchText(normalizedQuery),
    canonicalSearchText(getQueryCore(normalizedQuery)),
  ]);
}
...
function phraseTokensMatch(queryPhrase = "", candidatePhrase = "") {
  const queryTokens = canonicalSearchText(queryPhrase).split(" ").filter(Boolean);
  const candidateTokens = canonicalSearchText(candidatePhrase).split(" ").filter(Boolean);

  if (!queryTokens.length || !candidateTokens.length || queryTokens.length > candidateTokens.length) {
    return false;
  }

  return queryTokens.every((queryToken) =>
    candidateTokens.some((candidateToken) => isNearTokenMatch(queryToken, candidateToken))
  );
}
...
const CIRCUIT_ELEMENT_DIRECT_QUERIES = new Set([
  "resistor",
  "resistance",
  "capacitor",
  "capacitance",
...
function hasCircuitElementDirectIntent(normalizedQuery = "") {
  return CIRCUIT_ELEMENT_DIRECT_QUERIES.has(normalizedQuery) || CIRCUIT_ELEMENT_DIRECT_QUERIES.has(getQueryCore(normalizedQuery));
}
...
const curatedSearchItems = [
  {
    id: "paper-gate-2025-ece-solution",
    group: "Papers",
    type: "paper",
...
function scoreItem(item, queryTokens, normalizedQuery) {
  ...
  const coreQuery = getQueryCore(normalizedQuery);
  const compactQuery = normalizeCompact(normalizedQuery);
  ...
  if (coreQuery && coreQuery !== normalizedQuery) {
    const title = normalizeText(item.title || "");

    if (phraseTokensMatch(coreQuery, title)) {
      score += item.type === "subject" ? 190 : 130;
    } else if (phraseTokensMatch(coreQuery, scoreText)) {
      score += 85;
    }
  }
...
function getItemRankText(item) {
  return {
    title: normalizeText(item.title || ""),
    canonicalTitle: canonicalSearchText(item.title || ""),
    scoreText: item.scoreText || "",
    canonicalScoreText: canonicalSearchText(item.scoreText || ""),
  };
}

function matchesExactTitle(item, rankingQueries) {
  const rankText = getItemRankText(item);

  return rankingQueries.some(
    (query) =>
      query &&
      (rankText.title === query ||
        rankText.canonicalTitle === canonicalSearchText(query) ||
        rankText.scoreText === query ||
        rankText.canonicalScoreText === canonicalSearchText(query) ||
        phraseTokensMatch(query, rankText.title))
  );
}

function matchesExactSubject(item, rankingQueries) {
  return item.type === "subject" && matchesExactTitle(item, rankingQueries);
}

function matchesExactTopic(item, rankingQueries) {
  return ["topic", "subtopic", "chapter", "concept"].includes(item.type) && matchesExactTitle(item, rankingQueries);
}

function matchesNotesOrArticle(item) {
  return ["notes", "topic", "subtopic", "chapter", "concept"].includes(item.type);
}

function getRankingCategory(item, rankingQueries) {
  if (matchesExactSubject(item, rankingQueries)) {
    return SEARCH_RANK_ORDER.exactSubject;
  }

  if (matchesExactTopic(item, rankingQueries)) {
    return SEARCH_RANK_ORDER.exactTopic;
  }

  if (matchesNotesOrArticle(item)) {
    return SEARCH_RANK_ORDER.notesArticle;
  }

  if (item.type === "formula") {
    return SEARCH_RANK_ORDER.formula;
  }

  if (["paper", "pyq", "question"].includes(item.type)) {
    return SEARCH_RANK_ORDER.pyq;
  }

  if (["mcq", "practice"].includes(item.type)) {
    return SEARCH_RANK_ORDER.mcq;
  }

  return SEARCH_RANK_ORDER.semantic;
}
...
export function getSmartSearchResults(query = "", index = [], maxResults = 24) {
  ...
  const rankingQueries = getRankingQueryVariants(normalizedQuery);
  const directCircuitElementIntent = hasCircuitElementDirectIntent(normalizedQuery);

  return index
    .map((item) => ({
      ...item,
      matchScore: scoreItem(item, queryTokens, normalizedQuery),
      rankingCategory: getRankingCategory(item, rankingQueries),
    }))
    .filter((item) => item.matchScore > 0)
    .filter((item) => !directCircuitElementIntent || item.href === "/circuit-elements")
    .sort(
      (left, right) =>
        left.rankingCategory - right.rankingCategory ||
...
export function getPopularSearches(limit = 6) {
  return [
    { label: "BEL Probationary Engineer Papers", href: "/previous-year-papers/bel", group: "Papers" },
    { label: "BEL Previous Year Question Papers", href: "/previous-year-papers/bel", group: "Papers" },
    ...
    { label: "GATE Previous Year Question Papers", href: "/previous-year?exam=GATE", group: "Papers" },
