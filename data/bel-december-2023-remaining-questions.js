// Generated from public/papers/bel-probationary-engineer-electronics-official-paper-december-2023.pdf by scripts/extract-bel-december-2023-questions.mjs.
// The uploaded response PDF includes the remaining question text/options and the candidate's chosen option.
const belDecember2023RemainingQuestions = [
  {
    "_id": "bel-dec-2023-electronics-q17",
    "question": "Find the maximum power that can be delivered to the load resistor RL of the circuit shown in the official PDF figure.",
    "options": [
      "250/3 W",
      "100/3 W",
      "500/3 W",
      "200/3 W"
    ],
    "correctAnswer": "250/3 W",
    "explanation": "For maximum power transfer, RL equals the Thevenin resistance seen at A-B. The circuit gives VTh = 200/3 V and RTh = 40/3 ohms, so Pmax = VTh^2 / (4RTh) = 250/3 W.",
    "subject": "Question based on Electronics Engineering",
    "topic": "Official BEL Electronics Paper",
    "exam": [
      "BEL"
    ],
    "tags": [
      "official-paper"
    ],
    "year": 2023,
    "questionId": "630680512590",
    "optionIds": [
      "6306802003332",
      "6306802003333",
      "6306802003330",
      "6306802003331"
    ],
    "status": "Answered",
    "chosenOption": "1",
    "sourcePage": 15,
    "diagram": "bel-dec-2023-q17-circuit"
  },
  {
    "_id": "bel-dec-2023-electronics-q18",
    "question": "In the context of digital communications, what is the primary benefit of using a matchedfilter?",
    "options": [
      "It increases the transmission bandwidth.",
      "It reduces inter-symbol interference.",
      "It requires less power for the same performance.",
      "It enhances the phase stability of the received signal."
    ],
    "correctAnswer": "It reduces inter-symbol interference.",
    "explanation": "The marked BEL answer selects the matched-filter benefit as reducing inter-symbol interference for the received digital pulse decision.",
    "subject": "Question based on Electronics Engineering",
    "topic": "Official BEL Electronics Paper",
    "exam": [
      "BEL"
    ],
    "tags": [
      "official-paper"
    ],
    "year": 2023,
    "questionId": "630680512665",
    "optionIds": [
      "6306802003630",
      "6306802003633",
      "6306802003632",
      "6306802003631"
    ],
    "status": "Answered",
    "chosenOption": "4",
    "sourcePage": 15,
    "diagram": ""
  },
  {
    "_id": "bel-dec-2023-electronics-q19",
    "question": "For the circuit shown in the official PDF figure, determine the Thevenin equivalent circuit.",
    "options": [
      "ZTh = 4 ohms, VTh = 8 V",
      "ZTh = 2 ohms, VTh = 6 V",
      "ZTh = 3 ohms, VTh = 6 V",
      "ZTh = 3 ohms, VTh = 9 V"
    ],
    "correctAnswer": "ZTh = 4 ohms, VTh = 8 V",
    "explanation": "The marked BEL response identifies the Thevenin equivalent at terminals A-B as VTh = 8 V with ZTh = 4 ohms.",
    "subject": "Question based on Electronics Engineering",
    "topic": "Official BEL Electronics Paper",
    "exam": [
      "BEL"
    ],
    "tags": [
      "official-paper"
    ],
    "year": 2023,
    "questionId": "630680512589",
    "optionIds": [
      "6306802003328",
      "6306802003326",
      "6306802003327",
      "6306802003329"
    ],
    "status": "Answered",
    "chosenOption": "1",
    "sourcePage": 16,
    "diagram": "bel-dec-2023-q19-circuit"
  },
  {
    "_id": "bel-dec-2023-electronics-q20",
    "question": "The Fourier Transform of a rectangle function is:",
    "options": [
      "delta function",
      "exponential function",
      "unit step function",
      "sinc function"
    ],
    "correctAnswer": "sinc function",
    "explanation": "A rectangular pulse in one domain transforms to a sinc-shaped function in the Fourier domain.",
    "subject": "Question based on Electronics Engineering",
    "topic": "Official BEL Electronics Paper",
    "exam": [
      "BEL"
    ],
    "tags": [
      "official-paper"
    ],
    "year": 2023,
    "questionId": "630680512597",
    "optionIds": [
      "6306802003359",
      "6306802003361",
      "6306802003360",
      "6306802003358"
    ],
    "status": "Answered",
    "chosenOption": "4",
    "sourcePage": 16,
    "diagram": ""
  },
  {
    "_id": "bel-dec-2023-electronics-q21",
    "question": "How many satellite signals are required to calculate a precise 3D position using GPS?",
    "options": [
      "4",
      "3",
      "5",
      "2"
    ],
    "correctAnswer": "4",
    "explanation": "A GPS receiver needs four satellite signals to solve for three spatial coordinates and the receiver clock offset.",
    "subject": "Question based on Electronics Engineering",
    "topic": "Official BEL Electronics Paper",
    "exam": [
      "BEL"
    ],
    "tags": [
      "official-paper"
    ],
    "year": 2023,
    "questionId": "630680512674",
    "optionIds": [
      "6306802003668",
      "6306802003669",
      "6306802003667",
      "6306802003666"
    ],
    "status": "Answered",
    "chosenOption": "2",
    "sourcePage": 16,
    "diagram": ""
  },
  {
    "_id": "bel-dec-2023-electronics-q22",
    "question": "If a Nyquist plot encircles the point (-1,0) N times in the anticlockwise direction and has Z zeros of the open-loop system in the right-half plane, the number of poles (P) of the closed- loop system in the right-half plane will be:",
    "options": [
      "P = Z - N",
      "P = N - Z",
      "P = (N + Z)/2",
      "P = Z + N"
    ],
    "correctAnswer": "P = Z - N",
    "explanation": "With anticlockwise encirclements counted as N in the stated convention, the Nyquist relation for right-half-plane closed-loop poles is P = Z - N.",
    "subject": "Question based on Electronics Engineering",
    "topic": "Official BEL Electronics Paper",
    "exam": [
      "BEL"
    ],
    "tags": [
      "official-paper"
    ],
    "year": 2023,
    "questionId": "630680512648",
    "optionIds": [
      "6306802003562",
      "6306802003564",
      "6306802003565",
      "6306802003563"
    ],
    "status": "Answered",
    "chosenOption": "4",
    "sourcePage": 17,
    "diagram": ""
  },
  {
    "_id": "bel-dec-2023-electronics-q23",
    "question": "Thevenin's theorem is applicable to which of the following network types? I) Linear Networks II) Non-linear Networks III) Time Variant Networks",
    "options": [
      "All I, II and III",
      "Only II and III",
      "Only I",
      "Only I and III"
    ],
    "correctAnswer": "Only I",
    "explanation": "Thevenin's theorem is applied to linear networks. Non-linear and time-variant cases do not satisfy the standard theorem statement used here.",
    "subject": "Question based on Electronics Engineering",
    "topic": "Official BEL Electronics Paper",
    "exam": [
      "BEL"
    ],
    "tags": [
      "official-paper"
    ],
    "year": 2023,
    "questionId": "630680512591",
    "optionIds": [
      "6306802003337",
      "6306802003336",
      "6306802003334",
      "6306802003335"
    ],
    "status": "Answered",
    "chosenOption": "3",
    "sourcePage": 17,
    "diagram": ""
  },
  {
    "_id": "bel-dec-2023-electronics-q24",
    "question": "For an earth station transmitter input power of 25 dBW (10,000 W), with a back-off loss of 3 dB, a total branching and feeder loss of 3 dB, and a transmit antenna gain of 25 dB, determine the effective isotropic radiated power (EIRP).",
    "options": [
      "50 dBW",
      "57 dBW",
      "100 dBW",
      "44 dBW"
    ],
    "correctAnswer": "44 dBW",
    "explanation": "EIRP = input power - back-off loss - feeder loss + antenna gain = 25 - 3 - 3 + 25 = 44 dBW.",
    "subject": "Question based on Electronics Engineering",
    "topic": "Official BEL Electronics Paper",
    "exam": [
      "BEL"
    ],
    "tags": [
      "official-paper"
    ],
    "year": 2023,
    "questionId": "630680512683",
    "optionIds": [
      "6306802003704",
      "6306802003703",
      "6306802003705",
      "6306802003702"
    ],
    "status": "Answered",
    "chosenOption": "4",
    "sourcePage": 17,
    "diagram": ""
  },
  {
    "_id": "bel-dec-2023-electronics-q25",
    "question": "Which of the following statements is TRUE regarding the simplification of Boolean functions using Karnaugh maps?",
    "options": [
      "You can only group adjacent 1s in powers of two",
      "You cannot group 0s in the Karnaugh map",
      "You can group 1s and 0s together",
      "You need to group all the 1s in the map for a valid simplification"
    ],
    "correctAnswer": "You can only group adjacent 1s in powers of two",
    "explanation": "For SOP Karnaugh-map grouping, adjacent 1s are collected in groups sized 1, 2, 4, 8, and so on.",
    "subject": "Question based on Electronics Engineering",
    "topic": "Official BEL Electronics Paper",
    "exam": [
      "BEL"
    ],
    "tags": [
      "official-paper"
    ],
    "year": 2023,
    "questionId": "630680512635",
    "optionIds": [
      "6306802003512",
      "6306802003510",
      "6306802003511",
      "6306802003513"
    ],
    "status": "Answered",
    "chosenOption": "4",
    "sourcePage": 18,
    "diagram": ""
  },
  {
    "_id": "bel-dec-2023-electronics-q26",
    "question": "How many RST (restart) instructions are there in the 8085 microprocessor?",
    "options": [
      "10",
      "5",
      "8",
      "7"
    ],
    "correctAnswer": "8",
    "explanation": "The 8085 has eight restart instructions, RST 0 through RST 7.",
    "subject": "Question based on Electronics Engineering",
    "topic": "Official BEL Electronics Paper",
    "exam": [
      "BEL"
    ],
    "tags": [
      "official-paper"
    ],
    "year": 2023,
    "questionId": "630680512644",
    "optionIds": [
      "6306802003549",
      "6306802003546",
      "6306802003547",
      "6306802003548"
    ],
    "status": "Answered",
    "chosenOption": "2",
    "sourcePage": 18,
    "diagram": ""
  },
  {
    "_id": "bel-dec-2023-electronics-q27",
    "question": "What is the effect of increasing the bypass capacitor value in a single-stage BJT amplifier with emitter degeneration?",
    "options": [
      "Decrease in bandwidth",
      "Decrease in voltage gain",
      "Increase in voltage gain",
      "Increase in bandwidth"
    ],
    "correctAnswer": "Increase in voltage gain",
    "explanation": "A larger emitter bypass capacitor reduces AC emitter degeneration over the useful band, so the single-stage BJT amplifier voltage gain increases.",
    "subject": "Question based on Electronics Engineering",
    "topic": "Official BEL Electronics Paper",
    "exam": [
      "BEL"
    ],
    "tags": [
      "official-paper"
    ],
    "year": 2023,
    "questionId": "630680512620",
    "optionIds": [
      "6306802003453",
      "6306802003450",
      "6306802003451",
      "6306802003452"
    ],
    "status": "Answered",
    "chosenOption": "2",
    "sourcePage": 18,
    "diagram": ""
  },
  {
    "_id": "bel-dec-2023-electronics-q28",
    "question": "What is the execution time of the NOP instruction in an 8085 microprocessor with a 3 MHz clock frequency?",
    "options": [
      "7 μs",
      "5.32 μs",
      "1.33 μs",
      "6.67 μs"
    ],
    "correctAnswer": "1.33 μs",
    "explanation": "The 8085 NOP instruction uses 4 T-states. At 3 MHz, one clock period is 1/3 microsecond, so the execution time is 4/3 microseconds, approximately 1.33 microseconds.",
    "subject": "Question based on Electronics Engineering",
    "topic": "Official BEL Electronics Paper",
    "exam": [
      "BEL"
    ],
    "tags": [
      "official-paper"
    ],
    "year": 2023,
    "questionId": "630680512645",
    "optionIds": [
      "6306802003553",
      "6306802003551",
      "6306802003550",
      "6306802003552"
    ],
    "status": "Answered",
    "chosenOption": "4",
    "sourcePage": 19,
    "diagram": ""
  },
  {
    "_id": "bel-dec-2023-electronics-q29",
    "question": "In the context of the Channel Capacity Theorem, what role does channel bandwidth play in determining the capacity of a communication channel?",
    "options": [
      "Increasing channel bandwidth decreases channel capacity.",
      "Channel Bandwidth is directly proportional to channel capacity.",
      "Channel Bandwidth is inversely proportional to channel capacity.",
      "Channel Bandwidth has no effect on channel capacity."
    ],
    "correctAnswer": "Channel Bandwidth is directly proportional to channel capacity.",
    "explanation": "Shannon channel capacity increases with bandwidth for a fixed signal-to-noise ratio because C = B log2(1 + S/N).",
    "subject": "Question based on Electronics Engineering",
    "topic": "Official BEL Electronics Paper",
    "exam": [
      "BEL"
    ],
    "tags": [
      "official-paper"
    ],
    "year": 2023,
    "questionId": "630680512667",
    "optionIds": [
      "6306802003640",
      "6306802003638",
      "6306802003639",
      "6306802003641"
    ],
    "status": "Answered",
    "chosenOption": "2",
    "sourcePage": 19,
    "diagram": ""
  },
  {
    "_id": "bel-dec-2023-electronics-q30",
    "question": "For an underdamped second-order system, the damping ratio (zeta) is:",
    "options": [
      "Between 0 and 1",
      "Equal to 1",
      "Less than 0",
      "Greater than 1"
    ],
    "correctAnswer": "Between 0 and 1",
    "explanation": "A standard second-order system is underdamped when its damping ratio satisfies 0 < zeta < 1.",
    "subject": "Question based on Electronics Engineering",
    "topic": "Official BEL Electronics Paper",
    "exam": [
      "BEL"
    ],
    "tags": [
      "official-paper"
    ],
    "year": 2023,
    "questionId": "630680512649",
    "optionIds": [
      "6306802003568",
      "6306802003569",
      "6306802003567",
      "6306802003566"
    ],
    "status": "Answered",
    "chosenOption": "1",
    "sourcePage": 19,
    "diagram": "/papers/bel-probationary-engineer-electronics-official-paper-december-2023.pdf#page=19"
  },
  {
    "_id": "bel-dec-2023-electronics-q31",
    "question": "The bandwidth of an RLC parallel circuit is _________.",
    "options": [
      "Directly proportional to Q-factor",
      "Inversely proportional to inductance",
      "Directly proportional to resistance",
      "Inversely proportional to Q-factor"
    ],
    "correctAnswer": "Inversely proportional to Q-factor",
    "explanation": "For a resonant circuit, bandwidth is related to quality factor by BW = f0 / Q. Higher Q gives a narrower bandwidth.",
    "subject": "Question based on Electronics Engineering",
    "topic": "Official BEL Electronics Paper",
    "exam": [
      "BEL"
    ],
    "tags": [
      "official-paper"
    ],
    "year": 2023,
    "questionId": "630680512594",
    "optionIds": [
      "6306802003346",
      "6306802003349",
      "6306802003348",
      "6306802003347"
    ],
    "status": "Answered",
    "chosenOption": "4",
    "sourcePage": 20,
    "diagram": ""
  },
  {
    "_id": "bel-dec-2023-electronics-q32",
    "question": "Closed-loop control systems are characterized by which of the following? I) Direct control of system output without sensors. II) Use of feedback to reduce system errors. III)Independence from external environmental changes.",
    "options": [
      "Only I and II",
      "Only II",
      "All I, II and III",
      "Only III Only"
    ],
    "correctAnswer": "Only II",
    "explanation": "A closed-loop control system uses feedback to compare output with the reference and reduce error. The other statements do not define closed-loop operation.",
    "subject": "Question based on Electronics Engineering",
    "topic": "Official BEL Electronics Paper",
    "exam": [
      "BEL"
    ],
    "tags": [
      "official-paper"
    ],
    "year": 2023,
    "questionId": "630680512647",
    "optionIds": [
      "6306802003558",
      "6306802003559",
      "6306802003561",
      "6306802003560"
    ],
    "status": "Answered",
    "chosenOption": "3",
    "sourcePage": 20,
    "diagram": ""
  },
  {
    "_id": "bel-dec-2023-electronics-q33",
    "question": "Which of the following is the correct binary representation of the hexadecimal number 3A9?",
    "options": [
      "001110101001",
      "011110101001",
      "1110101001",
      "11110101001"
    ],
    "correctAnswer": "001110101001",
    "explanation": "Convert each hexadecimal digit to four bits: 3 = 0011, A = 1010, and 9 = 1001. Therefore 3A9 = 001110101001.",
    "subject": "Question based on Electronics Engineering",
    "topic": "Official BEL Electronics Paper",
    "exam": [
      "BEL"
    ],
    "tags": [
      "official-paper"
    ],
    "year": 2023,
    "questionId": "630680512631",
    "optionIds": [
      "6306802003495",
      "6306802003496",
      "6306802003494",
      "6306802003497"
    ],
    "status": "Answered",
    "chosenOption": "1",
    "sourcePage": 20,
    "diagram": ""
  },
  {
    "_id": "bel-dec-2023-electronics-q34",
    "question": "In phase modulation, what happens to the phase of the carrier wave when the amplitude of the modulating signal is zero?",
    "options": [
      "It becomes half of its maximum value",
      "It remains constant",
      "It becomes zero",
      "It becomes maximum"
    ],
    "correctAnswer": "It remains constant",
    "explanation": "In phase modulation, the phase deviation depends on the instantaneous modulating signal. With zero modulating amplitude, there is no phase deviation from the carrier reference.",
    "subject": "Question based on Electronics Engineering",
    "topic": "Official BEL Electronics Paper",
    "exam": [
      "BEL"
    ],
    "tags": [
      "official-paper"
    ],
    "year": 2023,
    "questionId": "630680512662",
    "optionIds": [
      "6306802003621",
      "6306802003618",
      "6306802003620",
      "6306802003619"
    ],
    "status": "Answered",
    "chosenOption": "2",
    "sourcePage": 21,
    "diagram": ""
  },
  {
    "_id": "bel-dec-2023-electronics-q35",
    "question": "Which biasing method provides the best thermal stability for a Bipolar Junction Transistor (BJT)?",
    "options": [
      "Voltage divider bias",
      "Collector feedback bias",
      "Emitter feedback bias",
      "Base bias"
    ],
    "correctAnswer": "Voltage divider bias",
    "explanation": "Voltage-divider bias with emitter stabilization makes the BJT operating point less sensitive to beta and temperature changes.",
    "subject": "Question based on Electronics Engineering",
    "topic": "Official BEL Electronics Paper",
    "exam": [
      "BEL"
    ],
    "tags": [
      "official-paper"
    ],
    "year": 2023,
    "questionId": "630680512618",
    "optionIds": [
      "6306802003444",
      "6306802003443",
      "6306802003445",
      "6306802003442"
    ],
    "status": "Answered",
    "chosenOption": "1",
    "sourcePage": 21,
    "diagram": ""
  },
  {
    "_id": "bel-dec-2023-electronics-q36",
    "question": "In a clipper circuit using a diode, what component is typically used in parallel with the diode to control the clipping threshold?",
    "options": [
      "An inductor",
      "A voltage source",
      "A capacitor",
      "A resistor"
    ],
    "correctAnswer": "A voltage source",
    "explanation": "A biased diode clipper uses a DC voltage source with the diode branch to set the level at which clipping starts.",
    "subject": "Question based on Electronics Engineering",
    "topic": "Official BEL Electronics Paper",
    "exam": [
      "BEL"
    ],
    "tags": [
      "official-paper"
    ],
    "year": 2023,
    "questionId": "630680512616",
    "optionIds": [
      "6306802003436",
      "6306802003437",
      "6306802003434",
      "6306802003435"
    ],
    "status": "Answered",
    "chosenOption": "2",
    "sourcePage": 21,
    "diagram": ""
  },
  {
    "_id": "bel-dec-2023-electronics-q37",
    "question": "Which of the following statements is NOT true about the Commutative Law in Boolean algebra?",
    "options": [
      "(A ∧ B) ∧ C = A ∧ (B ∧ C)",
      "A ∧ B = B ∧ A",
      "A ∨ B = B ∨ A",
      "(A ∨ B) ∨ C = A ∨ (B ∨ C)"
    ],
    "correctAnswer": "(A ∧ B) ∧ C = A ∧ (B ∧ C)",
    "explanation": "The marked option states an associative-law identity, not the commutative-law identity that swaps operand order.",
    "subject": "Question based on Electronics Engineering",
    "topic": "Official BEL Electronics Paper",
    "exam": [
      "BEL"
    ],
    "tags": [
      "official-paper"
    ],
    "year": 2023,
    "questionId": "630680512633",
    "optionIds": [
      "6306802003505",
      "6306802003503",
      "6306802003502",
      "6306802003504"
    ],
    "status": "Answered",
    "chosenOption": "1",
    "sourcePage": 22,
    "diagram": ""
  },
  {
    "_id": "bel-dec-2023-electronics-q38",
    "question": "Which of the following is a characteristic of a sinusoidal oscillator operating at its frequency of oscillation?",
    "options": [
      "The phase shift around the loop is maximum.",
      "The amplitude of the output signal is constant.",
      "The frequency of the output signal varies with time.",
      "The loop gain is less than one."
    ],
    "correctAnswer": "The amplitude of the output signal is constant.",
    "explanation": "At sustained sinusoidal oscillation, the loop satisfies the oscillation condition and the output amplitude settles to a constant value.",
    "subject": "Question based on Electronics Engineering",
    "topic": "Official BEL Electronics Paper",
    "exam": [
      "BEL"
    ],
    "tags": [
      "official-paper"
    ],
    "year": 2023,
    "questionId": "630680512627",
    "optionIds": [
      "6306802003479",
      "6306802003480",
      "6306802003478",
      "6306802003481"
    ],
    "status": "Answered",
    "chosenOption": "2",
    "sourcePage": 22,
    "diagram": ""
  },
  {
    "_id": "bel-dec-2023-electronics-q39",
    "question": "For maximum power transfer in AC circuits, the load impedance should be:",
    "options": [
      "Equal to source impedance",
      "Double of the source impedance",
      "Half of the source impedance",
      "Complex conjugate of the source impedance"
    ],
    "correctAnswer": "Complex conjugate of the source impedance",
    "explanation": "For maximum average power transfer in an AC circuit, the load impedance must equal the complex conjugate of the source or Thevenin impedance.",
    "subject": "Question based on Electronics Engineering",
    "topic": "Official BEL Electronics Paper",
    "exam": [
      "BEL"
    ],
    "tags": [
      "official-paper"
    ],
    "year": 2023,
    "questionId": "630680512588",
    "optionIds": [
      "6306802003322",
      "6306802003325",
      "6306802003324",
      "6306802003323"
    ],
    "status": "Answered",
    "chosenOption": "4",
    "sourcePage": 22,
    "diagram": ""
  },
  {
    "_id": "bel-dec-2023-electronics-q40",
    "question": "In PCM, what effect does an increase in the number of quantization levels have on the signal?",
    "options": [
      "It increases the bandwidth of the signal",
      "It decreases the bandwidth of the signal",
      "It increases the quantization noise",
      "It decreases the quantization noise"
    ],
    "correctAnswer": "It decreases the quantization noise",
    "explanation": "More PCM quantization levels reduce the quantization step size, so the quantization error and quantization noise decrease.",
    "subject": "Question based on Electronics Engineering",
    "topic": "Official BEL Electronics Paper",
    "exam": [
      "BEL"
    ],
    "tags": [
      "official-paper"
    ],
    "year": 2023,
    "questionId": "630680512664",
    "optionIds": [
      "6306802003626",
      "6306802003627",
      "6306802003629",
      "6306802003628"
    ],
    "status": "Answered",
    "chosenOption": "4",
    "sourcePage": 23,
    "diagram": ""
  },
  {
    "_id": "bel-dec-2023-electronics-q41",
    "question": "In a Bode plot, what is the significance of the gain crossover frequency?",
    "options": [
      "It indicates the unity gain frequency",
      "It corresponds to the phase crossover frequency",
      "It represents the damping ratio",
      "It is the frequency at which phase margin is minimum."
    ],
    "correctAnswer": "It indicates the unity gain frequency",
    "explanation": "The gain crossover frequency is the frequency at which the magnitude of the loop transfer function crosses unity, or 0 dB.",
    "subject": "Question based on Electronics Engineering",
    "topic": "Official BEL Electronics Paper",
    "exam": [
      "BEL"
    ],
    "tags": [
      "official-paper"
    ],
    "year": 2023,
    "questionId": "630680512653",
    "optionIds": [
      "6306802003582",
      "6306802003585",
      "6306802003584",
      "6306802003583"
    ],
    "status": "Answered",
    "chosenOption": "1",
    "sourcePage": 23,
    "diagram": ""
  },
  {
    "_id": "bel-dec-2023-electronics-q42",
    "question": "A system is said to be distortion less if its group delay is ______.",
    "options": [
      "Decreasing with frequency",
      "Zero",
      "Increasing with frequency",
      "Constant"
    ],
    "correctAnswer": "Constant",
    "explanation": "For distortionless transmission, all frequency components must experience the same delay. That corresponds to constant group delay.",
    "subject": "Question based on Electronics Engineering",
    "topic": "Official BEL Electronics Paper",
    "exam": [
      "BEL"
    ],
    "tags": [
      "official-paper"
    ],
    "year": 2023,
    "questionId": "630680512605",
    "optionIds": [
      "6306802003393",
      "6306802003390",
      "6306802003392",
      "6306802003391"
    ],
    "status": "Answered",
    "chosenOption": "1",
    "sourcePage": 23,
    "diagram": ""
  },
  {
    "_id": "bel-dec-2023-electronics-q43",
    "question": "In a Hamming (7,4) code, how many bits are used for the message and how many are used for error correction?",
    "options": [
      "4 bits for message, 3 bits for error correction",
      "7 bits for message, 4 bits for error correction",
      "4 bits for message, 7 bits for error correction",
      "3 bits for message, 4 bits for error correction"
    ],
    "correctAnswer": "4 bits for message, 3 bits for error correction",
    "explanation": "A Hamming (7,4) codeword has 7 total bits: 4 data bits and 3 parity bits used for error correction.",
    "subject": "Question based on Electronics Engineering",
    "topic": "Official BEL Electronics Paper",
    "exam": [
      "BEL"
    ],
    "tags": [
      "official-paper"
    ],
    "year": 2023,
    "questionId": "630680512669",
    "optionIds": [
      "6306802003648",
      "6306802003647",
      "6306802003646",
      "6306802003649"
    ],
    "status": "Answered",
    "chosenOption": "2",
    "sourcePage": 24,
    "diagram": ""
  },
  {
    "_id": "bel-dec-2023-electronics-q44",
    "question": "For an astable multivibrator using IC 555, if the values of R 1, R 2, and C are 47 kΩ, 100 kΩ, and 10 μF respectively, what is the time period of the output waveform?",
    "options": [
      "1.63 s",
      "1.71 s",
      "1.47 s",
      "3.33 s"
    ],
    "correctAnswer": "1.71 s",
    "explanation": "For a 555 astable multivibrator, T = 0.693 (R1 + 2R2) C. Substituting R1 = 47 kohm, R2 = 100 kohm, and C = 10 microfarad gives approximately 1.71 s.",
    "subject": "Question based on Electronics Engineering",
    "topic": "Official BEL Electronics Paper",
    "exam": [
      "BEL"
    ],
    "tags": [
      "official-paper"
    ],
    "year": 2023,
    "questionId": "630680512630",
    "optionIds": [
      "6306802003492",
      "6306802003493",
      "6306802003490",
      "6306802003491"
    ],
    "status": "Answered",
    "chosenOption": "2",
    "sourcePage": 24,
    "diagram": ""
  },
  {
    "_id": "bel-dec-2023-electronics-q45",
    "question": "The characteristic equation of a system is given as s^3 + 3s^2 + 7s + k = 0. Find the range of values of k for which the system would be stable.",
    "options": [
      "0 < k < 10",
      "0 < k < 21",
      "3 < k < 18",
      "4 < k < 10"
    ],
    "correctAnswer": "0 < k < 21",
    "explanation": "For the cubic polynomial s^3 + 3s^2 + 7s + k, the Routh stability conditions require k > 0 and 3 x 7 > k. Hence 0 < k < 21.",
    "subject": "Question based on Electronics Engineering",
    "topic": "Official BEL Electronics Paper",
    "exam": [
      "BEL"
    ],
    "tags": [
      "official-paper"
    ],
    "year": 2023,
    "questionId": "630680512650",
    "optionIds": [
      "6306802003572",
      "6306802003571",
      "6306802003570",
      "6306802003573"
    ],
    "status": "Answered",
    "chosenOption": "2",
    "sourcePage": 24,
    "diagram": ""
  },
  {
    "_id": "bel-dec-2023-electronics-q46",
    "question": "How many 4-to-1 multiplexers will be needed for implementing a 32-to-1 multiplexer?",
    "options": [
      "8",
      "9",
      "6",
      "11"
    ],
    "correctAnswer": "8",
    "explanation": "A 32-to-1 selection can be formed from eight 4-to-1 multiplexer blocks at the input grouping stage, matching the marked answer.",
    "subject": "Question based on Electronics Engineering",
    "topic": "Official BEL Electronics Paper",
    "exam": [
      "BEL"
    ],
    "tags": [
      "official-paper"
    ],
    "year": 2023,
    "questionId": "630680512637",
    "optionIds": [
      "6306802003518",
      "6306802003520",
      "6306802003519",
      "6306802003521"
    ],
    "status": "Answered",
    "chosenOption": "4",
    "sourcePage": 25,
    "diagram": ""
  },
  {
    "_id": "bel-dec-2023-electronics-q47",
    "question": "What is a crucial factor to consider in designing an active filter for high-frequency applications?",
    "options": [
      "The open-loop gain of the operational amplifier",
      "The value of the input resistor",
      "The slew rate of the operational amplifier",
      "The power supply voltage of the operational amplifier"
    ],
    "correctAnswer": "The slew rate of the operational amplifier",
    "explanation": "At high frequency and finite output swing, an op-amp needs sufficient slew rate to follow the signal without slew-induced distortion.",
    "subject": "Question based on Electronics Engineering",
    "topic": "Official BEL Electronics Paper",
    "exam": [
      "BEL"
    ],
    "tags": [
      "official-paper"
    ],
    "year": 2023,
    "questionId": "630680512624",
    "optionIds": [
      "6306802003466",
      "6306802003467",
      "6306802003469",
      "6306802003468"
    ],
    "status": "Answered",
    "chosenOption": "3",
    "sourcePage": 25,
    "diagram": ""
  },
  {
    "_id": "bel-dec-2023-electronics-q48",
    "question": "Poynting vector represents _______.",
    "options": [
      "The rate of energy transfer per unit area",
      "Magnetic field per unit area",
      "Electric field per unit area",
      "The rate of flux change per unit area"
    ],
    "correctAnswer": "The rate of energy transfer per unit area",
    "explanation": "The Poynting vector gives electromagnetic power flow density, which is the rate of energy transfer per unit area.",
    "subject": "Question based on Electronics Engineering",
    "topic": "Official BEL Electronics Paper",
    "exam": [
      "BEL"
    ],
    "tags": [
      "official-paper"
    ],
    "year": 2023,
    "questionId": "630680512682",
    "optionIds": [
      "6306802003700",
      "6306802003698",
      "6306802003699",
      "6306802003701"
    ],
    "status": "Answered",
    "chosenOption": "1",
    "sourcePage": 25,
    "diagram": ""
  },
  {
    "_id": "bel-dec-2023-electronics-q49",
    "question": "Which of the following latches will change its output only when the enable signal is active and there is a change in the input?",
    "options": [
      "SR Latch",
      "D Latch",
      "Gated D Latch",
      "T Latch"
    ],
    "correctAnswer": "Gated D Latch",
    "explanation": "A gated D latch is level-sensitive only while its enable input is active; during that interval its output follows changes at D.",
    "subject": "Question based on Electronics Engineering",
    "topic": "Official BEL Electronics Paper",
    "exam": [
      "BEL"
    ],
    "tags": [
      "official-paper"
    ],
    "year": 2023,
    "questionId": "630680512639",
    "optionIds": [
      "6306802003526",
      "6306802003528",
      "6306802003527",
      "6306802003529"
    ],
    "status": "Answered",
    "chosenOption": "3",
    "sourcePage": 26,
    "diagram": ""
  },
  {
    "_id": "bel-dec-2023-electronics-q50",
    "question": "In a sinusoidal oscillator, what is the phase shift around the loop at the frequency of oscillation?",
    "options": [
      "60 degrees",
      "180 degrees",
      "360 degrees",
      "90 degrees"
    ],
    "correctAnswer": "360 degrees",
    "explanation": "The Barkhausen phase condition requires the total loop phase shift at oscillation to be 0 degrees modulo 360 degrees.",
    "subject": "Question based on Electronics Engineering",
    "topic": "Official BEL Electronics Paper",
    "exam": [
      "BEL"
    ],
    "tags": [
      "official-paper"
    ],
    "year": 2023,
    "questionId": "630680512626",
    "optionIds": [
      "6306802003474",
      "6306802003477",
      "6306802003476",
      "6306802003475"
    ],
    "status": "Answered",
    "chosenOption": "2",
    "sourcePage": 26,
    "diagram": ""
  },
  {
    "_id": "bel-dec-2023-electronics-q51",
    "question": "Which parameter of the MOSFET is crucial in determining the unity-gain bandwidth of a single-stage amplifier?",
    "options": [
      "Oxide capacitance per unit area (C ox )",
      "Threshold voltage",
      "Transconductance (g m )",
      "Channel length modulation"
    ],
    "correctAnswer": "Transconductance (g m )",
    "explanation": "The MOSFET transconductance sets signal-current conversion and strongly affects the gain-bandwidth behavior of a single-stage amplifier.",
    "subject": "Question based on Electronics Engineering",
    "topic": "Official BEL Electronics Paper",
    "exam": [
      "BEL"
    ],
    "tags": [
      "official-paper"
    ],
    "year": 2023,
    "questionId": "630680512621",
    "optionIds": [
      "6306802003456",
      "6306802003454",
      "6306802003455",
      "6306802003457"
    ],
    "status": "Answered",
    "chosenOption": "3",
    "sourcePage": 26,
    "diagram": ""
  },
  {
    "_id": "bel-dec-2023-electronics-q52",
    "question": "Electricflux through a closed surface in free space is equal to:",
    "options": [
      "Electricfield strength times the area",
      "Charge enclosed divided by ε₀",
      "Charge enclosed times ε₀",
      "Electricflux density times the area"
    ],
    "correctAnswer": "Charge enclosed divided by ε₀",
    "explanation": "By Gauss's law, the total electric flux through a closed surface in free space equals the enclosed charge divided by ε₀.",
    "subject": "Question based on Electronics Engineering",
    "topic": "Official BEL Electronics Paper",
    "exam": [
      "BEL"
    ],
    "tags": [
      "official-paper"
    ],
    "year": 2023,
    "questionId": "630680512679",
    "optionIds": [
      "6306802003686",
      "6306802003687",
      "6306802003689",
      "6306802003688"
    ],
    "status": "Answered",
    "chosenOption": "2",
    "sourcePage": 27,
    "diagram": ""
  },
  {
    "_id": "bel-dec-2023-electronics-q53",
    "question": "Diffusion current in a semiconductor arises because of:",
    "options": [
      "Electricfield.",
      "Movement of holes.",
      "Concentration gradient of carriers.",
      "Applied voltage."
    ],
    "correctAnswer": "Applied voltage.",
    "explanation": "The official key shown for this BEL question marks applied voltage as the answer.",
    "subject": "Question based on Electronics Engineering",
    "topic": "Official BEL Electronics Paper",
    "exam": [
      "BEL"
    ],
    "tags": [
      "official-paper"
    ],
    "year": 2023,
    "questionId": "630680512607",
    "optionIds": [
      "6306802003400",
      "6306802003401",
      "6306802003399",
      "6306802003398"
    ],
    "status": "Answered",
    "chosenOption": "3",
    "sourcePage": 27,
    "diagram": ""
  },
  {
    "_id": "bel-dec-2023-electronics-q54",
    "question": "The impedance of an RLC series circuit at resonance is _______.",
    "options": [
      "Zero",
      "Purely resistive",
      "Purely reactive",
      "Maximum"
    ],
    "correctAnswer": "Purely resistive",
    "explanation": "At resonance in a series RLC circuit, inductive and capacitive reactances cancel, so the impedance is purely resistive.",
    "subject": "Question based on Electronics Engineering",
    "topic": "Official BEL Electronics Paper",
    "exam": [
      "BEL"
    ],
    "tags": [
      "official-paper"
    ],
    "year": 2023,
    "questionId": "630680512593",
    "optionIds": [
      "6306802003342",
      "6306802003344",
      "6306802003345",
      "6306802003343"
    ],
    "status": "Answered",
    "chosenOption": "2",
    "sourcePage": 27,
    "diagram": ""
  },
  {
    "_id": "bel-dec-2023-electronics-q55",
    "question": "Barkhausen's Criterion for sustained oscillations in an oscillator circuit states that:",
    "options": [
      "The loop gain must be zero.",
      "The phase shift around the loop must be 180 degrees.",
      "The product of the loop gain and the feedback factor must be one.",
      "The product of the loop gain and the feedback factor must be greater than one."
    ],
    "correctAnswer": "The product of the loop gain and the feedback factor must be one.",
    "explanation": "Barkhausen criterion for sustained oscillation requires loop gain magnitude |Aβ| = 1 with net phase shift 0 degrees or an integral multiple of 360 degrees.",
    "subject": "Question based on Electronics Engineering",
    "topic": "Official BEL Electronics Paper",
    "exam": [
      "BEL"
    ],
    "tags": [
      "official-paper"
    ],
    "year": 2023,
    "questionId": "630680512625",
    "optionIds": [
      "6306802003473",
      "6306802003471",
      "6306802003472",
      "6306802003470"
    ],
    "status": "Answered",
    "chosenOption": "3",
    "sourcePage": 28,
    "diagram": ""
  },
  {
    "_id": "bel-dec-2023-electronics-q56",
    "question": "For the transformation of a delta-connected network to a wye-connected network, the resistance between any two terminals in the wye-connection is:",
    "options": [
      "The resistance is equal to the product of the two delta resistances connected to the common terminal, divided by the sum of all three delta resistances.",
      "The resistance is equal to the difference between the highest and lowest delta resistances.",
      "The resistance is equal to the sum of the two delta resistances connected to the common terminal.",
      "The resistance is equal to the sum of the two delta resistances connected to the common terminal, divided by the total number of resistances in the delta network."
    ],
    "correctAnswer": "The resistance is equal to the product of the two delta resistances connected to the common terminal, divided by the sum of all three delta resistances.",
    "explanation": "In delta-to-wye conversion, each star resistance equals the product of the two adjacent delta resistances divided by the sum of all three delta resistances.",
    "subject": "Question based on Electronics Engineering",
    "topic": "Official BEL Electronics Paper",
    "exam": [
      "BEL"
    ],
    "tags": [
      "official-paper"
    ],
    "year": 2023,
    "questionId": "630680512587",
    "optionIds": [
      "6306802003318",
      "6306802003321",
      "6306802003319",
      "6306802003320"
    ],
    "status": "Answered",
    "chosenOption": "1",
    "sourcePage": 28,
    "diagram": ""
  },
  {
    "_id": "bel-dec-2023-electronics-q57",
    "question": "In the twin tub CMOS process, the p-well is used primarily for:",
    "options": [
      "Providing a substrate for resistors",
      "Isolating the pMOS transistors",
      "Connecting the bulk terminal to the positive supply",
      "Fabricating the nMOS transistors"
    ],
    "correctAnswer": "Fabricating the nMOS transistors",
    "explanation": "In a twin-tub CMOS process, the p-well region is used as the body region for fabricating nMOS transistors.",
    "subject": "Question based on Electronics Engineering",
    "topic": "Official BEL Electronics Paper",
    "exam": [
      "BEL"
    ],
    "tags": [
      "official-paper"
    ],
    "year": 2023,
    "questionId": "630680512613",
    "optionIds": [
      "6306802003424",
      "6306802003422",
      "6306802003425",
      "6306802003423"
    ],
    "status": "Not Answered",
    "chosenOption": "--",
    "sourcePage": 28,
    "diagram": ""
  },
  {
    "_id": "bel-dec-2023-electronics-q58",
    "question": "Convolution of an input signal with the impulse response of an LTI system provides:",
    "options": [
      "The phase response of the system",
      "The system's frequency response",
      "The system's transfer function",
      "The output of the system for that input"
    ],
    "correctAnswer": "The output of the system for that input",
    "explanation": "For an LTI system, output y(t) is the convolution of the input x(t) with the impulse response h(t).",
    "subject": "Question based on Electronics Engineering",
    "topic": "Official BEL Electronics Paper",
    "exam": [
      "BEL"
    ],
    "tags": [
      "official-paper"
    ],
    "year": 2023,
    "questionId": "630680512603",
    "optionIds": [
      "6306802003384",
      "6306802003383",
      "6306802003385",
      "6306802003382"
    ],
    "status": "Answered",
    "chosenOption": "4",
    "sourcePage": 29,
    "diagram": ""
  },
  {
    "_id": "bel-dec-2023-electronics-q59",
    "question": "How does white noise affect the performance of communication systems?",
    "options": [
      "It improves signal-to-noise ratio",
      "It has no effect on signal quality",
      "It degrades signal quality",
      "It enhances channel bandwidth"
    ],
    "correctAnswer": "It degrades signal quality",
    "explanation": "White noise adds random power across the signal bandwidth and reduces received signal quality by lowering effective SNR.",
    "subject": "Question based on Electronics Engineering",
    "topic": "Official BEL Electronics Paper",
    "exam": [
      "BEL"
    ],
    "tags": [
      "official-paper"
    ],
    "year": 2023,
    "questionId": "630680512657",
    "optionIds": [
      "6306802003601",
      "6306802003599",
      "6306802003600",
      "6306802003598"
    ],
    "status": "Answered",
    "chosenOption": "1",
    "sourcePage": 29,
    "diagram": ""
  },
  {
    "_id": "bel-dec-2023-electronics-q60",
    "question": "Calculate the reflection coe ffi cient when VSWR is 5.",
    "options": [
      "0",
      "1/2",
      "2/3",
      "1"
    ],
    "correctAnswer": "2/3",
    "explanation": "VSWR S = (1 + |Γ|) / (1 - |Γ|). For S = 5, |Γ| = (S - 1) / (S + 1) = 4/6 = 2/3.",
    "subject": "Question based on Electronics Engineering",
    "topic": "Official BEL Electronics Paper",
    "exam": [
      "BEL"
    ],
    "tags": [
      "official-paper"
    ],
    "year": 2023,
    "questionId": "630680512676",
    "optionIds": [
      "6306802003676",
      "6306802003674",
      "6306802003675",
      "6306802003677"
    ],
    "status": "Answered",
    "chosenOption": "3",
    "sourcePage": 29,
    "diagram": ""
  },
  {
    "_id": "bel-dec-2023-electronics-q61",
    "question": "In a BJT configured as a common-emitter amplifier, what role does the emitter resistor play in biasing?",
    "options": [
      "It isolates the input and output signals",
      "It provides negative feedback to stabilize the bias point",
      "It increases the input impedance",
      "It amplifies the input signal"
    ],
    "correctAnswer": "It provides negative feedback to stabilize the bias point",
    "explanation": "The emitter resistor provides emitter degeneration, creating negative feedback that stabilizes the BJT bias point against temperature and beta variations.",
    "subject": "Question based on Electronics Engineering",
    "topic": "Official BEL Electronics Paper",
    "exam": [
      "BEL"
    ],
    "tags": [
      "official-paper"
    ],
    "year": 2023,
    "questionId": "630680512619",
    "optionIds": [
      "6306802003448",
      "6306802003447",
      "6306802003446",
      "6306802003449"
    ],
    "status": "Answered",
    "chosenOption": "3",
    "sourcePage": 30,
    "diagram": ""
  },
  {
    "_id": "bel-dec-2023-electronics-q62",
    "question": "A broadside array operating at 50cm wavelength consists of 4 half wave dipoles spaced 25cm apart. Each element carries radio frequency current in the same phase and of magnitude 0.2 A. What will be the radiated power if the radiation resistance is 126 Ω?",
    "options": [
      "11 W",
      "10 W",
      "35 W",
      "30 W"
    ],
    "correctAnswer": "10 W",
    "explanation": "Using the marked official answer, the radiated power for the given four-element broadside array is 10 W.",
    "subject": "Question based on Electronics Engineering",
    "topic": "Official BEL Electronics Paper",
    "exam": [
      "BEL"
    ],
    "tags": [
      "official-paper"
    ],
    "year": 2023,
    "questionId": "630680512678",
    "optionIds": [
      "6306802003684",
      "6306802003685",
      "6306802003682",
      "6306802003683"
    ],
    "status": "Not Answered",
    "chosenOption": "--",
    "sourcePage": 30,
    "diagram": ""
  },
  {
    "_id": "bel-dec-2023-electronics-q63",
    "question": "Find the gain of a system with transfer function G(s) = (s + 2) / [(s + 1) (s + 3) (s + 4)] for a d.c. (zero frequency) input.",
    "options": [
      "1",
      "0",
      "1/8",
      "1/6"
    ],
    "correctAnswer": "1/6",
    "explanation": "For a d.c. input, set s = 0. G(0) = 2 / [(1)(3)(4)] = 2/12 = 1/6.",
    "subject": "Question based on Electronics Engineering",
    "topic": "Official BEL Electronics Paper",
    "exam": [
      "BEL"
    ],
    "tags": [
      "official-paper"
    ],
    "year": 2023,
    "questionId": "630680512652",
    "optionIds": [
      "6306802003581",
      "6306802003578",
      "6306802003580",
      "6306802003579"
    ],
    "status": "Answered",
    "chosenOption": "4",
    "sourcePage": 30,
    "diagram": ""
  },
  {
    "_id": "bel-dec-2023-electronics-q64",
    "question": "In a 4-bit binary adder, if the inputs are A = 1101 and B = 1011 with an initial carry of 0, what will be the sum andfinal carry?",
    "options": [
      "Sum: 11000, Carry: 0",
      "Sum: 1100, Carry: 1",
      "Sum: 10010, Carry: 0",
      "Sum: 1000, Carry: 1"
    ],
    "correctAnswer": "Sum: 1000, Carry: 1",
    "explanation": "1101 + 1011 = 11000 in binary. A 4-bit adder reports the lower four sum bits as 1000 with final carry 1.",
    "subject": "Question based on Electronics Engineering",
    "topic": "Official BEL Electronics Paper",
    "exam": [
      "BEL"
    ],
    "tags": [
      "official-paper"
    ],
    "year": 2023,
    "questionId": "630680512634",
    "optionIds": [
      "6306802003508",
      "6306802003507",
      "6306802003506",
      "6306802003509"
    ],
    "status": "Answered",
    "chosenOption": "4",
    "sourcePage": 31,
    "diagram": ""
  },
  {
    "_id": "bel-dec-2023-electronics-q65",
    "question": "The electricfield measured in the farfield of an antenna at a distance of 30 m is 1 Vm -1. The average power density at a distance of 300m from the antenna is:",
    "options": [
      "13.3 μW m -2",
      "26.6 μW m -2",
      "1.33 μW m -2",
      "2.66 μW m -2"
    ],
    "correctAnswer": "26.6 μW m -2",
    "explanation": "In the far field, electric field varies inversely with distance and power density varies as E^2. Increasing distance from 30 m to 300 m reduces E by 10 and power density by 100, giving 26.6 μW m^-2.",
    "subject": "Question based on Electronics Engineering",
    "topic": "Official BEL Electronics Paper",
    "exam": [
      "BEL"
    ],
    "tags": [
      "official-paper"
    ],
    "year": 2023,
    "questionId": "630680512685",
    "optionIds": [
      "6306802003713",
      "6306802003710",
      "6306802003712",
      "6306802003711"
    ],
    "status": "Not Answered",
    "chosenOption": "--",
    "sourcePage": 31,
    "diagram": ""
  },
  {
    "_id": "bel-dec-2023-electronics-q66",
    "question": "In semiconductor fabrication, the process used to pattern specific areas of a wafer for subsequent removal or deposition of materials is called:",
    "options": [
      "Epitaxy",
      "Annealing",
      "Diffusion",
      "Lithography"
    ],
    "correctAnswer": "Lithography",
    "explanation": "Lithography is the semiconductor fabrication step used to define or pattern selected wafer areas before etching, removal, or deposition.",
    "subject": "Question based on Electronics Engineering",
    "topic": "Official BEL Electronics Paper",
    "exam": [
      "BEL"
    ],
    "tags": [
      "official-paper"
    ],
    "year": 2023,
    "questionId": "630680512611",
    "optionIds": [
      "6306802003415",
      "6306802003416",
      "6306802003417",
      "6306802003414"
    ],
    "status": "Answered",
    "chosenOption": "1",
    "sourcePage": 31,
    "diagram": ""
  },
  {
    "_id": "bel-dec-2023-electronics-q67",
    "question": "A portable device requires low power consumption for extended battery life. Which logic family would be the most energy-e ffi cient choice for its digital circuitry?",
    "options": [
      "RTL",
      "ECL",
      "CMOS",
      "TTL"
    ],
    "correctAnswer": "CMOS",
    "explanation": "CMOS has very low static power consumption, making it the preferred logic family for battery-powered digital circuits.",
    "subject": "Question based on Electronics Engineering",
    "topic": "Official BEL Electronics Paper",
    "exam": [
      "BEL"
    ],
    "tags": [
      "official-paper"
    ],
    "year": 2023,
    "questionId": "630680512640",
    "optionIds": [
      "6306802003533",
      "6306802003532",
      "6306802003530",
      "6306802003531"
    ],
    "status": "Answered",
    "chosenOption": "3",
    "sourcePage": 32,
    "diagram": ""
  },
  {
    "_id": "bel-dec-2023-electronics-q68",
    "question": "What is the main purpose of the intermediate frequency (IF) stage in a superheterodyne receiver?",
    "options": [
      "To amplify the received signal",
      "To facilitate easier and more selectivefiltering",
      "To demodulate the signal",
      "To convert the signal to a lower frequency"
    ],
    "correctAnswer": "To facilitate easier and more selectivefiltering",
    "explanation": "The IF stage converts signals to a fixed intermediate frequency, where stable, selective filtering is easier to implement.",
    "subject": "Question based on Electronics Engineering",
    "topic": "Official BEL Electronics Paper",
    "exam": [
      "BEL"
    ],
    "tags": [
      "official-paper"
    ],
    "year": 2023,
    "questionId": "630680512663",
    "optionIds": [
      "6306802003622",
      "6306802003624",
      "6306802003623",
      "6306802003625"
    ],
    "status": "Answered",
    "chosenOption": "2",
    "sourcePage": 32,
    "diagram": ""
  },
  {
    "_id": "bel-dec-2023-electronics-q69",
    "question": "What is the main cause of GPS signal degradation in urban environments?",
    "options": [
      "Satellite geometry",
      "Tropospheric effects",
      "Ionospheric effects",
      "Multipath propagation"
    ],
    "correctAnswer": "Multipath propagation",
    "explanation": "In urban areas, GPS signals reflect from buildings and other structures, producing multipath propagation and degrading accuracy.",
    "subject": "Question based on Electronics Engineering",
    "topic": "Official BEL Electronics Paper",
    "exam": [
      "BEL"
    ],
    "tags": [
      "official-paper"
    ],
    "year": 2023,
    "questionId": "630680512675",
    "optionIds": [
      "6306802003673",
      "6306802003672",
      "6306802003671",
      "6306802003670"
    ],
    "status": "Not Answered",
    "chosenOption": "--",
    "sourcePage": 32,
    "diagram": ""
  },
  {
    "_id": "bel-dec-2023-electronics-q70",
    "question": "In a Hamming code, if one of the received bits is incorrect, which of the following is true?",
    "options": [
      "The syndrome vector will be all zeros.",
      "The error will always be in a data bit.",
      "The syndrome vector will point to the position of the error.",
      "The error cannot be corrected."
    ],
    "correctAnswer": "The syndrome vector will point to the position of the error.",
    "explanation": "For a single-bit error in a Hamming code, the nonzero syndrome identifies the bit position that must be corrected.",
    "subject": "Question based on Electronics Engineering",
    "topic": "Official BEL Electronics Paper",
    "exam": [
      "BEL"
    ],
    "tags": [
      "official-paper"
    ],
    "year": 2023,
    "questionId": "630680512670",
    "optionIds": [
      "6306802003650",
      "6306802003653",
      "6306802003651",
      "6306802003652"
    ],
    "status": "Answered",
    "chosenOption": "3",
    "sourcePage": 33,
    "diagram": ""
  },
  {
    "_id": "bel-dec-2023-electronics-q71",
    "question": "What is the relationship between the modulation index (m) and the amplitudes of the carrier (Ac) and message (Am) signals in amplitude modulation?",
    "options": [
      "M = 1/ (Ac.Am)",
      "m = Am/ Ac",
      "M = Ac. Am",
      "M = Ac/ Am"
    ],
    "correctAnswer": "m = Am/ Ac",
    "explanation": "In amplitude modulation, the modulation index is the ratio of message-signal amplitude to carrier amplitude.",
    "subject": "Question based on Electronics Engineering",
    "topic": "Official BEL Electronics Paper",
    "exam": [
      "BEL"
    ],
    "tags": [
      "official-paper"
    ],
    "year": 2023,
    "questionId": "630680512658",
    "optionIds": [
      "6306802003605",
      "6306802003603",
      "6306802003604",
      "6306802003602"
    ],
    "status": "Answered",
    "chosenOption": "2",
    "sourcePage": 33,
    "diagram": ""
  },
  {
    "_id": "bel-dec-2023-electronics-q72",
    "question": "How does a positive clipper circuit operate when the diode is forward biased?",
    "options": [
      "It blocks all positive parts of the input signal.",
      "It allows all positive parts of the input signal to pass.",
      "It allows negative as well as positive parts of the input signal to pass.",
      "It blocks all negative parts of the input signal."
    ],
    "correctAnswer": "It blocks all positive parts of the input signal.",
    "explanation": "A positive clipper removes or clips the positive portion of the waveform when the diode conducts.",
    "subject": "Question based on Electronics Engineering",
    "topic": "Official BEL Electronics Paper",
    "exam": [
      "BEL"
    ],
    "tags": [
      "official-paper"
    ],
    "year": 2023,
    "questionId": "630680512617",
    "optionIds": [
      "6306802003441",
      "6306802003440",
      "6306802003439",
      "6306802003438"
    ],
    "status": "Answered",
    "chosenOption": "1",
    "sourcePage": 33,
    "diagram": ""
  },
  {
    "_id": "bel-dec-2023-electronics-q73",
    "question": "In an n-channel enhancement-type MOSFET, what happens when V GS < V T?",
    "options": [
      "The MOSFET is in the triode region.",
      "The MOSFET is in the saturation mode.",
      "The MOSFET conducts maximum current.",
      "The MOSFET is in the cutoff mode and no currentflows between the drain and the source."
    ],
    "correctAnswer": "The MOSFET is in the cutoff mode and no currentflows between the drain and the source.",
    "explanation": "For an n-channel enhancement MOSFET, when VGS is below threshold voltage VT, no inversion channel forms and the device is cut off.",
    "subject": "Question based on Electronics Engineering",
    "topic": "Official BEL Electronics Paper",
    "exam": [
      "BEL"
    ],
    "tags": [
      "official-paper"
    ],
    "year": 2023,
    "questionId": "630680512615",
    "optionIds": [
      "6306802003433",
      "6306802003431",
      "6306802003430",
      "6306802003432"
    ],
    "status": "Answered",
    "chosenOption": "4",
    "sourcePage": 34,
    "diagram": ""
  },
  {
    "_id": "bel-dec-2023-electronics-q74",
    "question": "In a single-stage MOSFET amplifier, what role does the Miller capacitance play?",
    "options": [
      "It decreases the high-frequency gain.",
      "It increases the high-frequency gain.",
      "It stabilizes the DC operating point.",
      "It enhances the linearity of the amplifier."
    ],
    "correctAnswer": "It decreases the high-frequency gain.",
    "explanation": "Miller capacitance increases the effective input capacitance and rolls off the amplifier response at high frequencies.",
    "subject": "Question based on Electronics Engineering",
    "topic": "Official BEL Electronics Paper",
    "exam": [
      "BEL"
    ],
    "tags": [
      "official-paper"
    ],
    "year": 2023,
    "questionId": "630680512622",
    "optionIds": [
      "6306802003458",
      "6306802003459",
      "6306802003460",
      "6306802003461"
    ],
    "status": "Answered",
    "chosenOption": "1",
    "sourcePage": 34,
    "diagram": ""
  },
  {
    "_id": "bel-dec-2023-electronics-q75",
    "question": "In a BJT, the base-emitter junction is _______ biased and the collector-base junction is _______ biased in active mode of operation.",
    "options": [
      "forward; reverse",
      "reverse; reverse",
      "reverse; forward",
      "forward; forward"
    ],
    "correctAnswer": "forward; reverse",
    "explanation": "In active mode, a BJT has its base-emitter junction forward biased and collector-base junction reverse biased.",
    "subject": "Question based on Electronics Engineering",
    "topic": "Official BEL Electronics Paper",
    "exam": [
      "BEL"
    ],
    "tags": [
      "official-paper"
    ],
    "year": 2023,
    "questionId": "630680512610",
    "optionIds": [
      "6306802003411",
      "6306802003410",
      "6306802003413",
      "6306802003412"
    ],
    "status": "Answered",
    "chosenOption": "1",
    "sourcePage": 34,
    "diagram": ""
  },
  {
    "_id": "bel-dec-2023-electronics-q76",
    "question": "In the context of LEDs, what does the term \"quantum e ffi ciency\" refer to?",
    "options": [
      "The e ffi ciency of converting electric energy into heat.",
      "The e ffi ciency of converting electric energy into magneticfields.",
      "The ratio of the number of photons emitted into free space per second to the number of electrons injected into LED per second.",
      "The number of photons absorbed per second."
    ],
    "correctAnswer": "The ratio of the number of photons emitted into free space per second to the number of electrons injected into LED per second.",
    "explanation": "LED quantum efficiency measures emitted photons per injected carrier, indicating how effectively electrical carriers are converted into light.",
    "subject": "Question based on Electronics Engineering",
    "topic": "Official BEL Electronics Paper",
    "exam": [
      "BEL"
    ],
    "tags": [
      "official-paper"
    ],
    "year": 2023,
    "questionId": "630680512614",
    "optionIds": [
      "6306802003427",
      "6306802003428",
      "6306802003426",
      "6306802003429"
    ],
    "status": "Answered",
    "chosenOption": "3",
    "sourcePage": 35,
    "diagram": ""
  },
  {
    "_id": "bel-dec-2023-electronics-q77",
    "question": "In a binary-to-gray code converter, if the binary input is 1011, what will be the gray code output?",
    "options": [
      "1110",
      "1001",
      "1010",
      "1101"
    ],
    "correctAnswer": "1110",
    "explanation": "For binary 1011, the Gray-code MSB is 1 and each next bit is the XOR of adjacent binary bits: 1, 1, 1, 0.",
    "subject": "Question based on Electronics Engineering",
    "topic": "Official BEL Electronics Paper",
    "exam": [
      "BEL"
    ],
    "tags": [
      "official-paper"
    ],
    "year": 2023,
    "questionId": "630680512636",
    "optionIds": [
      "6306802003515",
      "6306802003517",
      "6306802003514",
      "6306802003516"
    ],
    "status": "Answered",
    "chosenOption": "1",
    "sourcePage": 35,
    "diagram": ""
  },
  {
    "_id": "bel-dec-2023-electronics-q78",
    "question": "In a shift register, if a binary number 1101 is initially stored and the register is made to shift two times to the right, which of the following would be the content of the register?",
    "options": [
      "1110",
      "1101",
      "0011",
      "0110"
    ],
    "correctAnswer": "0011",
    "explanation": "Right shifting 1101 by two places with zeros shifted in gives 0011.",
    "subject": "Question based on Electronics Engineering",
    "topic": "Official BEL Electronics Paper",
    "exam": [
      "BEL"
    ],
    "tags": [
      "official-paper"
    ],
    "year": 2023,
    "questionId": "630680512642",
    "optionIds": [
      "6306802003538",
      "6306802003541",
      "6306802003540",
      "6306802003539"
    ],
    "status": "Answered",
    "chosenOption": "3",
    "sourcePage": 35,
    "diagram": ""
  },
  {
    "_id": "bel-dec-2023-electronics-q79",
    "question": "Which of the following statements is true regarding the bandwidth of an amplitude- modulated signal?",
    "options": [
      "The bandwidth is dependent on the modulation index.",
      "The bandwidth is twice the highest frequency of the message signal.",
      "The bandwidth is equal to the sum of the carrier frequency and twice the highest frequency of the message signal.",
      "The bandwidth is equal to the carrier frequency."
    ],
    "correctAnswer": "The bandwidth is twice the highest frequency of the message signal.",
    "explanation": "For standard AM, the transmission bandwidth is twice the highest modulating frequency because upper and lower sidebands are produced.",
    "subject": "Question based on Electronics Engineering",
    "topic": "Official BEL Electronics Paper",
    "exam": [
      "BEL"
    ],
    "tags": [
      "official-paper"
    ],
    "year": 2023,
    "questionId": "630680512659",
    "optionIds": [
      "6306802003609",
      "6306802003608",
      "6306802003606",
      "6306802003607"
    ],
    "status": "Answered",
    "chosenOption": "2",
    "sourcePage": 36,
    "diagram": ""
  },
  {
    "_id": "bel-dec-2023-electronics-q80",
    "question": "In a monostable multivibrator using IC 555, what is the purpose of the external trigger?",
    "options": [
      "To control the duty cycle",
      "To reset the output to a low state",
      "To initiate the timing interval",
      "To toggle the output to the reverse state"
    ],
    "correctAnswer": "To initiate the timing interval",
    "explanation": "In a 555 monostable circuit, the external trigger starts the one-shot pulse and initiates the timing interval.",
    "subject": "Question based on Electronics Engineering",
    "topic": "Official BEL Electronics Paper",
    "exam": [
      "BEL"
    ],
    "tags": [
      "official-paper"
    ],
    "year": 2023,
    "questionId": "630680512629",
    "optionIds": [
      "6306802003489",
      "6306802003488",
      "6306802003486",
      "6306802003487"
    ],
    "status": "Not Answered",
    "chosenOption": "--",
    "sourcePage": 36,
    "diagram": ""
  },
  {
    "_id": "bel-dec-2023-electronics-q81",
    "question": "During the process of Interpolation, Upsampling is followed by ________.",
    "options": [
      "Decimation",
      "Amplitude scaling",
      "Low-passfiltering",
      "Quantization"
    ],
    "correctAnswer": "Low-passfiltering",
    "explanation": "Interpolation inserts samples during upsampling, then uses low-pass filtering to remove imaging components.",
    "subject": "Question based on Electronics Engineering",
    "topic": "Official BEL Electronics Paper",
    "exam": [
      "BEL"
    ],
    "tags": [
      "official-paper"
    ],
    "year": 2023,
    "questionId": "630680512602",
    "optionIds": [
      "6306802003379",
      "6306802003381",
      "6306802003380",
      "6306802003378"
    ],
    "status": "Answered",
    "chosenOption": "2",
    "sourcePage": 36,
    "diagram": ""
  },
  {
    "_id": "bel-dec-2023-electronics-q82",
    "question": "For a source with M equiprobable symbols, what is the formula for calculating entropy (H)?",
    "options": [
      "H = Mlog 2 (M)",
      "H = log 2 (M)",
      "H = -log 2 (M)",
      "H = -Mlog 2 (M)"
    ],
    "correctAnswer": "H = log 2 (M)",
    "explanation": "For M equiprobable symbols, each has probability 1/M, so entropy H = -M(1/M)log2(1/M) = log2(M).",
    "subject": "Question based on Electronics Engineering",
    "topic": "Official BEL Electronics Paper",
    "exam": [
      "BEL"
    ],
    "tags": [
      "official-paper"
    ],
    "year": 2023,
    "questionId": "630680512668",
    "optionIds": [
      "6306802003645",
      "6306802003644",
      "6306802003643",
      "6306802003642"
    ],
    "status": "Answered",
    "chosenOption": "4",
    "sourcePage": 37,
    "diagram": ""
  },
  {
    "_id": "bel-dec-2023-electronics-q83",
    "question": "In a rectangular waveguide, the TE10 mode has ______.",
    "options": [
      "one variation in the broader dimension and none in the narrower dimension",
      "nofield variation in both dimensions",
      "one variation in the narrower dimension and none in the broader dimension",
      "multiple variations in the broader dimension"
    ],
    "correctAnswer": "one variation in the broader dimension and none in the narrower dimension",
    "explanation": "In TE10 mode, the first index indicates one half-wave variation along the broader dimension and the second index indicates no variation along the narrower dimension.",
    "subject": "Question based on Electronics Engineering",
    "topic": "Official BEL Electronics Paper",
    "exam": [
      "BEL"
    ],
    "tags": [
      "official-paper"
    ],
    "year": 2023,
    "questionId": "630680512684",
    "optionIds": [
      "6306802003708",
      "6306802003709",
      "6306802003706",
      "6306802003707"
    ],
    "status": "Answered",
    "chosenOption": "1",
    "sourcePage": 37,
    "diagram": ""
  },
  {
    "_id": "bel-dec-2023-electronics-q84",
    "question": "When the modulation index of an FM signal is small (much less than one), the bandwidth of the FM signal can be approximated by which of the following?",
    "options": [
      "The sum of the carrier frequency and the frequency deviation.",
      "Twice the sum of the carrier frequency and the modulating frequency.",
      "Twice the highest frequency of the modulating signal.",
      "Twice the frequency deviation."
    ],
    "correctAnswer": "Twice the highest frequency of the modulating signal.",
    "explanation": "For narrowband FM with a very small modulation index, the bandwidth is approximated as twice the highest modulating frequency.",
    "subject": "Question based on Electronics Engineering",
    "topic": "Official BEL Electronics Paper",
    "exam": [
      "BEL"
    ],
    "tags": [
      "official-paper"
    ],
    "year": 2023,
    "questionId": "630680512661",
    "optionIds": [
      "6306802003616",
      "6306802003617",
      "6306802003615",
      "6306802003614"
    ],
    "status": "Answered",
    "chosenOption": "3",
    "sourcePage": 37,
    "diagram": ""
  },
  {
    "_id": "bel-dec-2023-electronics-q85",
    "question": "In a non-inverting amplifier using an operational amplifier, the voltage gain is determined by which of the following?",
    "options": [
      "The open-loop gain of the operational amplifier",
      "The product of the input and feedback resistors",
      "The ratio of the feedback resistor to the input resistor",
      "The sum of the input and feedback resistors"
    ],
    "correctAnswer": "The ratio of the feedback resistor to the input resistor",
    "explanation": "The closed-loop gain of a non-inverting op-amp is set by the resistor ratio, commonly A_v = 1 + Rf/Rin.",
    "subject": "Question based on Electronics Engineering",
    "topic": "Official BEL Electronics Paper",
    "exam": [
      "BEL"
    ],
    "tags": [
      "official-paper"
    ],
    "year": 2023,
    "questionId": "630680512623",
    "optionIds": [
      "6306802003463",
      "6306802003464",
      "6306802003462",
      "6306802003465"
    ],
    "status": "Answered",
    "chosenOption": "3",
    "sourcePage": 38,
    "diagram": ""
  },
  {
    "_id": "bel-dec-2023-electronics-q86",
    "question": "Which of the following conditions is responsible for the occurrence of Aliasing?",
    "options": [
      "The sampling frequency is too low",
      "The signal is periodic",
      "The sampling frequency is too high",
      "The signal is non-periodic"
    ],
    "correctAnswer": "The sampling frequency is too low",
    "explanation": "Aliasing occurs when the sampling frequency is below the Nyquist rate, causing spectral overlap.",
    "subject": "Question based on Electronics Engineering",
    "topic": "Official BEL Electronics Paper",
    "exam": [
      "BEL"
    ],
    "tags": [
      "official-paper"
    ],
    "year": 2023,
    "questionId": "630680512598",
    "optionIds": [
      "6306802003363",
      "6306802003364",
      "6306802003362",
      "6306802003365"
    ],
    "status": "Answered",
    "chosenOption": "1",
    "sourcePage": 38,
    "diagram": ""
  },
  {
    "_id": "bel-dec-2023-electronics-q87",
    "question": "The fan-out of a TTL gate is mainly determined by:",
    "options": [
      "The output resistance of the gate",
      "The input resistance of the gate",
      "The capacitance of the load",
      "The power supply voltage"
    ],
    "correctAnswer": "The input resistance of the gate",
    "explanation": "TTL fan-out is limited by how much input current the driven gates require, which is represented here by the input resistance/load of the gates.",
    "subject": "Question based on Electronics Engineering",
    "topic": "Official BEL Electronics Paper",
    "exam": [
      "BEL"
    ],
    "tags": [
      "official-paper"
    ],
    "year": 2023,
    "questionId": "630680512638",
    "optionIds": [
      "6306802003522",
      "6306802003523",
      "6306802003525",
      "6306802003524"
    ],
    "status": "Not Answered",
    "chosenOption": "--",
    "sourcePage": 38,
    "diagram": ""
  },
  {
    "_id": "bel-dec-2023-electronics-q88",
    "question": "In line-of-sight communication, the Fresnel Zone is important for:",
    "options": [
      "Determining the minimum height of the antenna.",
      "Ensuring that the signal path is free from obstacles.",
      "Decreasing the required transmission power.",
      "Ensuring proper modulation."
    ],
    "correctAnswer": "Ensuring that the signal path is free from obstacles.",
    "explanation": "Fresnel-zone clearance is needed in line-of-sight links to reduce obstruction losses and maintain reliable propagation.",
    "subject": "Question based on Electronics Engineering",
    "topic": "Official BEL Electronics Paper",
    "exam": [
      "BEL"
    ],
    "tags": [
      "official-paper"
    ],
    "year": 2023,
    "questionId": "630680512673",
    "optionIds": [
      "6306802003663",
      "6306802003662",
      "6306802003665",
      "6306802003664"
    ],
    "status": "Not Answered",
    "chosenOption": "--",
    "sourcePage": 39,
    "diagram": ""
  },
  {
    "_id": "bel-dec-2023-electronics-q89",
    "question": "At the boundary between two dielectric materials, which of the following is continuous?",
    "options": [
      "Magneticfield H",
      "The normal component of the Electric displacementfield (D)",
      "Current density J",
      "Electricfield E"
    ],
    "correctAnswer": "The normal component of the Electric displacementfield (D)",
    "explanation": "At a dielectric boundary with no free surface charge, the normal component of electric displacement density D is continuous.",
    "subject": "Question based on Electronics Engineering",
    "topic": "Official BEL Electronics Paper",
    "exam": [
      "BEL"
    ],
    "tags": [
      "official-paper"
    ],
    "year": 2023,
    "questionId": "630680512681",
    "optionIds": [
      "6306802003696",
      "6306802003695",
      "6306802003697",
      "6306802003694"
    ],
    "status": "Answered",
    "chosenOption": "2",
    "sourcePage": 39,
    "diagram": ""
  },
  {
    "_id": "bel-dec-2023-electronics-q90",
    "question": "Which of the following is the primary purpose of IFFT?",
    "options": [
      "Compress the frequency domain signal",
      "Decompress the time-domain signal",
      "Reconstruct the frequency components from time domain signal",
      "Convert frequency domain signal to time domain"
    ],
    "correctAnswer": "Convert frequency domain signal to time domain",
    "explanation": "The inverse fast Fourier transform converts frequency-domain samples back into the corresponding time-domain signal.",
    "subject": "Question based on Electronics Engineering",
    "topic": "Official BEL Electronics Paper",
    "exam": [
      "BEL"
    ],
    "tags": [
      "official-paper"
    ],
    "year": 2023,
    "questionId": "630680512600",
    "optionIds": [
      "6306802003371",
      "6306802003372",
      "6306802003373",
      "6306802003370"
    ],
    "status": "Not Answered",
    "chosenOption": "--",
    "sourcePage": 39,
    "diagram": ""
  },
  {
    "_id": "bel-dec-2023-electronics-q91",
    "question": "In nodal analysis, the number of nodes to be considered for KCL equations is equal to:",
    "options": [
      "Total nodes",
      "Total nodes - 1",
      "Total nodes - 2",
      "Total nodes + 1"
    ],
    "correctAnswer": "Total nodes - 1",
    "explanation": "One node is chosen as the reference node, so KCL equations are written for the remaining total nodes minus one.",
    "subject": "Question based on Electronics Engineering",
    "topic": "Official BEL Electronics Paper",
    "exam": [
      "BEL"
    ],
    "tags": [
      "official-paper"
    ],
    "year": 2023,
    "questionId": "630680512586",
    "optionIds": [
      "6306802003315",
      "6306802003316",
      "6306802003314",
      "6306802003317"
    ],
    "status": "Answered",
    "chosenOption": "2",
    "sourcePage": 40,
    "diagram": ""
  },
  {
    "_id": "bel-dec-2023-electronics-q92",
    "question": "The Region of Convergence (ROC) of the Z transform for which of the following discrete-time sequences is NOT |z| > 1?",
    "options": [
      "cos ωn u(n)",
      "1/n; n > 0",
      "δ(n)",
      "u(n)"
    ],
    "correctAnswer": "δ(n)",
    "explanation": "The Z-transform of δ(n) is 1, so its ROC is the entire z-plane, not the region |z| > 1.",
    "subject": "Question based on Electronics Engineering",
    "topic": "Official BEL Electronics Paper",
    "exam": [
      "BEL"
    ],
    "tags": [
      "official-paper"
    ],
    "year": 2023,
    "questionId": "630680512601",
    "optionIds": [
      "6306802003376",
      "6306802003374",
      "6306802003377",
      "6306802003375"
    ],
    "status": "Answered",
    "chosenOption": "2",
    "sourcePage": 40,
    "diagram": "/papers/bel-probationary-engineer-electronics-official-paper-december-2023.pdf#page=40"
  },
  {
    "_id": "bel-dec-2023-electronics-q93",
    "question": "How many T-states are required for the opcode fetch cycle in 8085 microprocessor?",
    "options": [
      "1",
      "3",
      "4",
      "2"
    ],
    "correctAnswer": "4",
    "explanation": "In the 8085, the opcode fetch machine cycle takes 4 T-states.",
    "subject": "Question based on Electronics Engineering",
    "topic": "Official BEL Electronics Paper",
    "exam": [
      "BEL"
    ],
    "tags": [
      "official-paper"
    ],
    "year": 2023,
    "questionId": "630680512643",
    "optionIds": [
      "6306802003542",
      "6306802003545",
      "6306802003543",
      "6306802003544"
    ],
    "status": "Answered",
    "chosenOption": "2",
    "sourcePage": 40,
    "diagram": ""
  },
  {
    "_id": "bel-dec-2023-electronics-q94",
    "question": "In a Bode plot, how is the order of a system determined from the phase plot?",
    "options": [
      "By examining the gain margin",
      "By counting the number of phase crossings",
      "By the location of the magnitude peak",
      "By analysing the slope of the phase plot"
    ],
    "correctAnswer": "By counting the number of phase crossings",
    "explanation": "The marked official answer determines the system order from the phase plot by counting phase crossings.",
    "subject": "Question based on Electronics Engineering",
    "topic": "Official BEL Electronics Paper",
    "exam": [
      "BEL"
    ],
    "tags": [
      "official-paper"
    ],
    "year": 2023,
    "questionId": "630680512646",
    "optionIds": [
      "6306802003556",
      "6306802003554",
      "6306802003557",
      "6306802003555"
    ],
    "status": "Not Answered",
    "chosenOption": "--",
    "sourcePage": 41,
    "diagram": ""
  },
  {
    "_id": "bel-dec-2023-electronics-q95",
    "question": "What is the length of the linear convolution of two sequences of length M and N?",
    "options": [
      "M + N - 1",
      "M + N",
      "M*N",
      "M*N - 1"
    ],
    "correctAnswer": "M + N - 1",
    "explanation": "The linear convolution of sequences with lengths M and N has length M + N - 1.",
    "subject": "Question based on Electronics Engineering",
    "topic": "Official BEL Electronics Paper",
    "exam": [
      "BEL"
    ],
    "tags": [
      "official-paper"
    ],
    "year": 2023,
    "questionId": "630680512604",
    "optionIds": [
      "6306802003389",
      "6306802003388",
      "6306802003386",
      "6306802003387"
    ],
    "status": "Answered",
    "chosenOption": "1",
    "sourcePage": 41,
    "diagram": "/papers/bel-probationary-engineer-electronics-official-paper-december-2023.pdf#page=41"
  },
  {
    "_id": "bel-dec-2023-electronics-q96",
    "question": "In asynchronous counters, theflip- fl ops (except thefirst) are triggered by:",
    "options": [
      "The output of the precedingflip- fl op",
      "A reset signal",
      "An external control signal",
      "The clock signal at the same time"
    ],
    "correctAnswer": "The output of the precedingflip- fl op",
    "explanation": "In an asynchronous counter, each flip-flop after the first is clocked by the output of the preceding flip-flop.",
    "subject": "Question based on Electronics Engineering",
    "topic": "Official BEL Electronics Paper",
    "exam": [
      "BEL"
    ],
    "tags": [
      "official-paper"
    ],
    "year": 2023,
    "questionId": "630680512641",
    "optionIds": [
      "6306802003535",
      "6306802003537",
      "6306802003536",
      "6306802003534"
    ],
    "status": "Answered",
    "chosenOption": "1",
    "sourcePage": 41,
    "diagram": ""
  },
  {
    "_id": "bel-dec-2023-electronics-q97",
    "question": "A 10.0 V stabilised power supply is required to be produced from a 12 V DC power supply input source. If the maximum power rating of the Zener diode used is 4 W, calculate the maximum Current through zener diode.",
    "options": [
      "200 mA",
      "150 mA",
      "400 mA",
      "100 mA"
    ],
    "correctAnswer": "400 mA",
    "explanation": "Maximum Zener current is I = P/Vz = 4 W / 10 V = 0.4 A = 400 mA.",
    "subject": "Question based on Electronics Engineering",
    "topic": "Official BEL Electronics Paper",
    "exam": [
      "BEL"
    ],
    "tags": [
      "official-paper"
    ],
    "year": 2023,
    "questionId": "630680512609",
    "optionIds": [
      "6306802003407",
      "6306802003408",
      "6306802003406",
      "6306802003409"
    ],
    "status": "Answered",
    "chosenOption": "3",
    "sourcePage": 42,
    "diagram": ""
  },
  {
    "_id": "bel-dec-2023-electronics-q98",
    "question": "A Zener diode is specially designed to ________.",
    "options": [
      "Operate in the reverse breakdown region.",
      "Increase the forward current.",
      "Operate in the forward-biased region.",
      "Enhance power dissipation."
    ],
    "correctAnswer": "Operate in the reverse breakdown region.",
    "explanation": "A Zener diode is designed to operate safely in reverse breakdown for voltage regulation.",
    "subject": "Question based on Electronics Engineering",
    "topic": "Official BEL Electronics Paper",
    "exam": [
      "BEL"
    ],
    "tags": [
      "official-paper"
    ],
    "year": 2023,
    "questionId": "630680512608",
    "optionIds": [
      "6306802003404",
      "6306802003403",
      "6306802003402",
      "6306802003405"
    ],
    "status": "Answered",
    "chosenOption": "1",
    "sourcePage": 42,
    "diagram": ""
  },
  {
    "_id": "bel-dec-2023-electronics-q99",
    "question": "The integral form of Faraday's law is given by:",
    "options": [
      "The electromotive force around a closed loop is proportional to the electricfield",
      "The magneticfield around a closed loop is equal to the electricflux",
      "The electromotive force around a closed loop is equal to the time rate of change of magneticflux through the loop",
      "The magneticflux through a closed surface is zero"
    ],
    "correctAnswer": "The electromotive force around a closed loop is equal to the time rate of change of magneticflux through the loop",
    "explanation": "Faraday's law states that the induced emf around a closed loop equals the negative time rate of change of magnetic flux through the loop; the marked option gives this relationship.",
    "subject": "Question based on Electronics Engineering",
    "topic": "Official BEL Electronics Paper",
    "exam": [
      "BEL"
    ],
    "tags": [
      "official-paper"
    ],
    "year": 2023,
    "questionId": "630680512680",
    "optionIds": [
      "6306802003692",
      "6306802003693",
      "6306802003690",
      "6306802003691"
    ],
    "status": "Answered",
    "chosenOption": "3",
    "sourcePage": 42,
    "diagram": ""
  },
  {
    "_id": "bel-dec-2023-electronics-q100",
    "question": "In a monostable multivibrator using IC 555, if the resistance R is 100 kΩ and the capacitance C is 10 μF, what is the pulse width of the output?",
    "options": [
      "1.1 ms",
      "1.1 s",
      "2.2 s",
      "2.2 ms"
    ],
    "correctAnswer": "1.1 s",
    "explanation": "For a 555 monostable multivibrator, pulse width T = 1.1RC. With R = 100 kΩ and C = 10 μF, T = 1.1 x 100000 x 10^-5 = 1.1 s.",
    "subject": "Question based on Electronics Engineering",
    "topic": "Official BEL Electronics Paper",
    "exam": [
      "BEL"
    ],
    "tags": [
      "official-paper"
    ],
    "year": 2023,
    "questionId": "630680512628",
    "optionIds": [
      "6306802003482",
      "6306802003483",
      "6306802003484",
      "6306802003485"
    ],
    "status": "Answered",
    "chosenOption": "2",
    "sourcePage": 43,
    "diagram": ""
  }
];

export default belDecember2023RemainingQuestions;
