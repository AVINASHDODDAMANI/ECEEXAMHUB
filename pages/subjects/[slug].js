...
function SubjectBlueprintSection({
  subject,
  chapterMeta,
  steps = [],
  concepts = [],
  ...
  const syllabusItems = stepsToSyllabus(steps).slice(0, 8);
  const importantTopics = (chapterMeta?.keyConcepts || concepts.map((concept) => concept.shortTitle || concept.title))
    .filter(Boolean)
    .slice(0, 8);
  const weightageValue = learningMeta?.weightage || learningMeta?.subjectWeightage || "Core subject";
  const topTopicLinks = learningTopics.slice(0, 4);
  const formulaTopic = concepts.find((concept) => Array.isArray(concept.formulas) && concept.formulas.length > 0);
  const playlistTopics = learningTopics.slice(0, 5);
  const bookTracks = [
    {
      title: "Concept textbook pass",
      text: `Read ${subject.title} chapter-wise for definitions, derivations, and physical meaning before heavy practice.`,
    },
    {
      title: "Solved example pass",
      text: "Use worked examples to learn method selection, assumptions, and standard exam steps.",
    },
    {
      title: "Objective revision pass",
      text: "Finish with PYQs, short notes, and formula revision so the chapter becomes exam-usable.",
    },
  ];
  const resourceGroups = [
    {
      title: "Notes",
      description: `Structured notes for ${subject.title} with chapter-wise coverage and quick revision flow.`,
      href: notesHref,
      action: "Open Notes",
      tone: "border-portal-200 bg-portal-50/60",
    },
    {
      title: "PYQs",
      description: "Use previous year questions to spot repeated patterns and weightage-driven asks.",
      href: `/previous-year?search=${encodeURIComponent(subject.search)}`,
      action: "Solve PYQs",
      tone: "border-orange-200 bg-orange-50/70",
    },
    {
      title: "Formula Sheets",
      description: formulaTopic
        ? `Start formula revision from ${formulaTopic.shortTitle || formulaTopic.title} and keep adding chapter-wise results.`
        : "Build one formula sheet after every finished topic so revision stays compact.",
      href: `/learn?search=${encodeURIComponent(`${subject.search} formula`)}`,
      action: "Revise Formulas",
      tone: "border-emerald-200 bg-emerald-50/70",
    },
    {
      title: "Practice",
      description: "Move from theory to MCQs and topic practice right after each completed chapter.",
  ...
  return (
    <section className="mt-5 rounded-[30px] border border-slate-200 bg-white p-4 shadow-panel sm:p-5">
      <div className="max-w-3xl">
        <p className="text-[11px] font-bold uppercase tracking-[0.16em] text-portal-700">
          Curated subject structure
        </p>
        <h2 className="mt-2 text-2xl font-bold tracking-tight text-slate-950 sm:text-3xl">
          {subject.title} Study Blueprint
        </h2>
        <p className="mt-3 text-sm leading-7 text-slate-700 sm:text-base">
          This page is organized as a guided subject hub, not a resource dump. Start with the overview and syllabus, focus on high-yield topics, then move into notes, formulas, PYQs, and revision strategy in that order.
        </p>
      </div>
      ...
      <div className="mt-6 grid gap-5 xl:grid-cols-[1.05fr_0.95fr]">
        <div className="grid gap-5">
          <div className="rounded-2xl border border-slate-200 bg-slate-50/70 p-4">
            ...
            <p className="mt-3 text-sm leading-7 text-slate-700">
              {chapterMeta?.subtitle || subjectSummary}
            </p>
          </div>

          <div className="rounded-2xl border border-slate-200 bg-white p-4">
            <div className="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
              <h3 className="text-base font-bold text-slate-950">Syllabus</h3>
              <span className="text-xs font-semibold uppercase tracking-[0.12em] text-slate-500">
                {syllabusItems.length} study blocks
              </span>
            </div>
            <div className="mt-4 grid gap-3">
              {syllabusItems.map((item, index) => (
                <div key={item} className="flex gap-3 rounded-xl border border-slate-200 bg-slate-50 px-3 py-3">
                  <span className="mt-0.5 inline-flex h-6 w-6 flex-none items-center justify-center rounded-full bg-white text-xs font-extrabold text-portal-700">
                    {index + 1}
                  </span>
          ...
          <div className="grid gap-4 lg:grid-cols-2">
            <div className="rounded-2xl border border-slate-200 bg-white p-4">
              <h3 className="text-base font-bold text-slate-950">Important Topics</h3>
              <div className="mt-4 flex flex-wrap gap-2">
                {importantTopics.map((item) => (
                  <span key={item} className="rounded-full bg-portal-50 px-3 py-2 text-xs font-bold text-portal-700">
                    {item}
                  </span>
                ))}
              </div>
            ...
            <div className="rounded-2xl border border-slate-200 bg-white p-4">
              <h3 className="text-base font-bold text-slate-950">Weightage</h3>
              <p className="mt-3 text-2xl font-extrabold text-slate-950">{weightageValue}</p>
              <p className="mt-2 text-sm leading-6 text-slate-700">
                Use this as a prioritization signal when you decide how much revision depth and PYQ practice this subject deserves in your schedule.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="grid gap-5">
          <div className="rounded-2xl border border-slate-200 bg-white p-4">
            <h3 className="text-base font-bold text-slate-950">Resource Groups</h3>
            <div className="mt-4 grid gap-3">
              {resourceGroups.map((group) => (
                <Link
                  key={group.title}
                  href={group.href}
                  className={`rounded-2xl border p-4 transition hover:-translate-y-0.5 hover:shadow-sm ${group.tone}`}
                >
                  <p className="text-sm font-extrabold text-slate-950">{group.title}</p>
                  <p className="mt-2 text-sm leading-6 text-slate-700">{group.description}</p>
                  <span className="mt-3 inline-flex text-xs font-extrabold uppercase tracking-[0.12em] text-portal-700">
                    {group.action} -&gt;
                  </span>
                </Link>
              ))}
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-4">
            <h3 className="text-base font-bold text-slate-950">Suggested Playlist Order</h3>
            <p className="mt-2 text-sm leading-6 text-slate-700">
              Treat these as your topic playlist sequence so learning happens in a coherent order instead of random jumps.
            </p>
            <div className="mt-4 grid gap-3">
              {playlistTopics.map((topic, index) => (
                <Link
                  key={topic.href}
                  href={topic.href}
                  className="flex items-start gap-3 rounded-xl border border-slate-200 bg-slate-50 px-3 py-3 transition hover:border-portal-200 hover:bg-portal-50"
                >
                  <span className="mt-0.5 inline-flex h-6 w-6 flex-none items-center justify-center rounded-full bg-white text-xs font-extrabold text-portal-700">
                    {index + 1}
                  </span>
                  <div>
                    <p className="text-sm font-bold text-slate-950">{topic.title}</p>
                    <p className="mt-1 text-xs font-semibold uppercase tracking-[0.12em] text-slate-500">
                      {topic.chapterTitle || topic.subjectName}
                    </p>
                  </div>
                </Link>
              ))}
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-4">
            <h3 className="text-base font-bold text-slate-950">Books</h3>
            <div className="mt-4 grid gap-3">
              {bookTracks.map((item) => (
          ...
          <div className="rounded-2xl border border-slate-200 bg-slate-50/80 p-4">
            <h3 className="text-base font-bold text-slate-950">Preparation Strategy</h3>
            <p className="mt-2 text-sm leading-7 text-slate-700">
              {chapterMeta?.studyTip ||
                `Study ${subject.title} in chapter order, revise formulas after each topic, then solve PYQs before moving to the next subject.`}
            </p>
            {topTopicLinks.length ? (
              <div className="mt-4 flex flex-wrap gap-2">
                {topTopicLinks.map((topic) => (
                  <Link
                    key={topic.href}
                    href={topic.href}
                    className="rounded-full border border-white bg-white px-3 py-2 text-xs font-bold text-portal-700 transition hover:border-portal-200"
                  >
                    {topic.title}
                  </Link>
                ))}
...
function stepsToSyllabus(steps = []) {
  return steps.flatMap((step) => {
    const points = Array.isArray(step.points) ? step.points : [];
    return points.length ? points : [step.title];
  });
}
...
export function getSubjectTheoryProps(subjectSlug, extraProps = {}) {
  ...
  return {
    props: {
      ...
      learningMeta: {
        learningSubjectSlug,
        weightage: learningSubject?.weightage || "",
